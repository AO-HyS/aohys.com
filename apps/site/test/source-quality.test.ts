...
describe("public site source quality", () => {
  it("keeps localized production copy behind typed i18n catalogs", () => {
    ...
    expect(localizedSources).not.toMatch(
      /(?:locale|draft\.locale|localeContent\.locale)\s*===\s*["'](?:en|es)["']/,
    );
    expect(read("src/i18n.ts")).toContain("satisfies Record<Locale, UiCopy>");
    expect(read("../../packages/content-graph/src/i18n.ts")).toContain(
  ...
  it("keeps design tokens, fonts, and graph-backed routing wired", () => {
    ...
    const engineeringPracticeDiagram = read(
      "public/images/proof/engineering-practice-release-cycle.svg",
    );
    ...
    const dashboardClasses = read(
      "../../apps/dashboard/src/lib/dashboard-classes.ts",
    );
    ...
    expect(styleClasses).toContain(
      "[&_a:focus-visible]:[outline:3px_solid_var(--color-focus)]",
    );
    expect(
      styleClasses.includes(
        "focus-visible:" + "outline-" + "[3px_solid_var(--color-focus)]",
      ),
    ).toBe(false);
    ...
    expect(enDictionary).toContain(
      "Independent product engineering by Alejandro Ortiz Corro.",
    );
    expect(esDictionary).toContain(
    ...
    expect(proofMedia).toContain(
      "dashboardMedia?.alt ?? evidenceAlt ?? staticMedia?.alt",
    );
    expect(proofImage).toContain('candidate.addEventListener("error"');
    expect(proofImage).toContain('candidate.removeAttribute("srcset")');
    expect(proofImage).toContain('candidate.removeAttribute("sizes")');
    ...
    expect(webglScene).not.toMatch(
      /three|BoxGeometry|TorusGeometry|WebGLRenderer|sampler2D/,
    );
    expect(
      existsSync(
        path.join(siteRoot, "src/components/sunlit/sunlit-three-runtime.ts"),
      ),
    ).toBe(false);
    ...
    expect(publicContentPage).toContain(
      'data-service-pattern={["new", "team", "modernize"][index]}',
    );
    expect(publicContentPage).toContain("sunlit-architecture-topology");
    expect(publicContentPage).toContain("sunlit-project-brief");
    expect(publicContentPage).toContain(
      ".sunlit-profile-projects article > :is(h3, p, ul)",
    );
    expect(publicContentPage).toContain(
      'font-family: "Atkinson Hyperlegible Next Variable", "Atkinson Hyperlegible Next"',
    );
    ...
    expect(webglScene).toContain(
      "Math.min(window.devicePixelRatio || 1, width < 720 ? 1 : 1.5)",
    );
    ...
    expect(publicContentPage).toContain(
      ".sunlit-related > div { display: grid;",
    );
    expect(publicContentPage).not.toContain(
      ".sunlit-related > div { display: flex; overflow-x: auto;",
    );
    ...
    expect(contentGraph).toContain(
      "/images/proof/enterprise-systems-map-v2.svg",
    );
    expect(contentGraph).toContain(
      "/images/proof/engineering-practice-release-cycle.svg",
    );
    expect(contentGraph).toContain(
    ...
    for (const asset of [
      ...
      "public/images/proof/eteria-garden-blue-table-480.webp",
      "public/images/proof/eteria-garden-blue-table-800.webp",
      ...
      expect(
        existsSync(path.join(siteRoot, asset)),
        `${asset} is missing`,
      ).toBe(true);
    ...
    expect(
      [
        ...new Set(
          engineeringPracticeDiagram
            .match(/#[0-9a-fA-F]{6}/g)
            ?.map((color) => color.toUpperCase()),
        ),
      ].sort(),
    ).toEqual(["#473C33", "#ABC270", "#FDA769", "#FEC868", "#FFFFFF"].sort());
    expect(contentGraph).not.toMatch(
      /aohys-pixel|pixel-product-landscape|pixel-hills|pixel-lake/,
    );
    ...
    expect(enDictionary).toContain(
      "Client work and operational data stay private",
    );
    ...
    expect(dashboardFunction).toContain(
      'await import("../../apps/site/src/dashboard-access.js")',
    );
    expect(authFunction).toContain(
      'await import("../../../apps/site/src/auth-proxy.js")',
    );
    expect(cspFunction).toContain(
      'await import("../../apps/site/src/csp-reporting.js")',
    );
