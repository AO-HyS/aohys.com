...
import { resolveProjectMediaPreview } from "./projects-model";
import {
  useProjectsWorkflow,
  type MediaUploadIssue,
  type NewProjectInput,
  type ProjectFormState,
} from "./projects-workflow";
...
const evidenceStatuses: DashboardEvidenceStatus[] = [
  "missing",
  "sanitized",
  "published",
];
...
function ProjectsWorkspace({ content }: { content: ProjectsContent }) {
  ...
  const selectedProjectId =
    requestedProjectId &&
    content.projects.some((project) => project.contentId === requestedProjectId)
      ? requestedProjectId
  ...
  const activeDeleteKey = deleteRequest
    ? `${deleteRequest.mediaId}:delete`
    : null;
  ...
  return (
    <div className={dashboardClass.workspace}>
      ...
      {isCreateOpen ? (
        <NewProjectCard
          existingContentIds={content.projects.map(
            (project) => project.contentId,
          )}
    ...
    <AlertDialog open={Boolean(issue)} onOpenChange={onOpenChange}>
      <AlertDialogContent className="sm:max-w-md">
        <AlertDialogHeader>
          ...
          <AlertDialogTitle>
            {issue?.title ?? "Image upload failed"}
          </AlertDialogTitle>
          <AlertDialogDescription>
    ...
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            This removes the media record from the dashboard. Publish afterwards
            if the public Astro image should change.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
      ...
      <TabsList className={dashboardClass.projectTabsList}>
        {projects.map((project) => (
          <TabsTrigger
            key={project.contentId}
            value={project.contentId}
            className={dashboardClass.projectTabTrigger}
          >
            <span className={dashboardClass.projectTabCopy}>
              <span className={dashboardClass.projectTabTitle}>
                {project.title}
              </span>
              <small className={dashboardClass.projectTabMeta}>
...
function NewProjectCard({
  ...
  function update<K extends keyof NewProjectInput>(
    ...
    setForm((current) => ({
      ...current,
      [key]: value,
      ...(key === "title" && !current.contentKey
        ? { contentKey: slugifyProjectTitle(String(value)) }
        : {}),
      ...(key === "title" && !current.englishSlug
        ? { englishSlug: slugifyProjectTitle(String(value)) }
        : {}),
      ...(key === "spanishTitle" && !current.spanishSlug
        ? { spanishSlug: slugifyProjectTitle(String(value)) }
        : {}),
    }));
  }
  ...
  const slugExists =
    isSafeProjectSlug(contentKey) && existingContentIds.includes(nextContentId);
    ...
    <Card className={dashboardClass.cardShadow}>
      <CardHeader className={dashboardClass.cardHeaderRule}>
        <div>
          ...
          <CardDescription>
            Add a new public case-study draft. It appears in the dashboard
            immediately and in Astro after publish/build.
          </CardDescription>
        </div>
      </CardHeader>
      <CardContent>
        ...
        >
          <FieldGroup className={dashboardClass.newProjectForm}>
            ...
            <LabeledInput
              ...
              onValueChange={(value) => update("spanishTitle", value)}
            />
            <LabeledInput
            ...
            <LabeledInput
              label="English slug"
              value={form.englishSlug}
              placeholder="project-slug"
              onValueChange={(value) =>
                update("englishSlug", slugifyProjectTitle(value))
              }
            />
            <LabeledInput
              label="Spanish slug"
              value={form.spanishSlug}
              placeholder="proyecto-slug"
              onValueChange={(value) =>
                update("spanishSlug", slugifyProjectTitle(value))
              }
            />
            <LabeledSelect
              label="Status"
              value={form.status}
              onValueChange={(value) =>
                update("status", value as DashboardCaseStudyStatus)
              }
              options={caseStudyStatuses.map((status) => ({
                value: status,
          ...
          <Tabs defaultValue="en" className={dashboardClass.localeEditor}>
            <CardHeader className={dashboardClass.localeHeader}>
              <div>
                ...
                <CardDescription>
                  Choose the language, edit the draft, then save before
                  publishing.
                </CardDescription>
              </div>
              <CardAction>
                <TabsList className={dashboardClass.localeTabsList}>
                  {project.locales.map((localeContent) => (
                    <TabsTrigger
                      key={localeContent.locale}
                      value={localeContent.locale}
                    >
                      {
                        getSharedI18n(localeContent.locale).dashboard
                          .languageName
                      }
                    </TabsTrigger>
                  ))}
              ...
              {project.locales.map((localeContent) => (
                ...
                >
                  <ProjectLocaleForm
                    ...
                    isSaving={
                      savingKey ===
                      `${project.contentId}:${localeContent.locale}`
                    }
  ...
  return (
    <Card className={dashboardClass.projectSummaryCard}>
      <CardHeader className={dashboardClass.projectSummaryHeader}>
        <div className="min-w-0">
          <CardTitle className={dashboardClass.projectSummaryTitle}>
            {project.title}
          </CardTitle>
        ...
        <CardAction className={dashboardClass.projectSummaryBadges}>
          ...
          <Badge
            variant={
              project.evidenceStatus === "published" ? "default" : "outline"
            }
          >
            {formatReferenceState(project.evidenceStatus)}
          </Badge>
          <Badge variant="outline">
            {project.sitemapIncluded ? "In sitemap" : "Noindex"}
          </Badge>
        </CardAction>
      </CardHeader>
      <CardContent className={dashboardClass.projectSummaryContent}>
        {project.projectUrl ? (
          <a
            className={dashboardClass.projectSummaryUrl}
          ...
          <span
            className={cn(
              dashboardClass.projectSummaryUrl,
              dashboardClass.projectSummaryUrlEmpty,
            )}
          >
            No public URL yet
          </span>
        ...
        <div className={dashboardClass.publishBar}>
          <div className={dashboardClass.publishMeta}>
            <span>
              Saved {latestDraft ? formatDate(latestDraft.updatedAt) : "never"}
            </span>
            <span>
              Published{" "}
...
function ProjectLocaleForm({
  ...
  const initialForm = {
    ...
    seoDescription:
      localeContent.draft?.seoDescription ?? localeContent.seoDescription,
    ...
    achievements:
      localeContent.draft?.achievements ?? localeContent.achievements,
    structureNotes:
      localeContent.draft?.structureNotes ?? localeContent.structureNotes,
  };
  ...
  return (
    ...
    >
      <div className={dashboardClass.projectFormHeader}>
        <div>
          <h2>
            {getSharedI18n(localeContent.locale).dashboard.contentHeading}
          </h2>
          <p>
            {localeContent.path} ·{" "}
            {localeContent.draft
              ? "private dashboard draft"
      ...
      <FieldSet>
        ...
        <FieldGroup>
          <div className={dashboardClass.formGrid3}>
            ...
            />
            <LabeledSelect
              label="Public link state"
              value={form.evidenceStatus}
              onValueChange={(value) =>
                update("evidenceStatus", value as DashboardEvidenceStatus)
              }
              options={evidenceStatuses.map((status) => ({
                value: status,
                label: formatReferenceState(status),
              }))}
            />
            <LabeledInput
              label="Project URL"
              ...
              onValueChange={(value) => update("projectUrl", value)}
            />
          </div>
          <LabeledInput
            label="Title"
          ...
          <LabeledInput
            ...
            value={form.localizedSlug ?? ""}
            onValueChange={(value) =>
              update("localizedSlug", slugifyProjectTitle(value))
            }
          />
          <LabeledTextarea
            label="Summary"
          ...
          <LabeledTextarea
            ...
            description="Write for search results and humans. No vague proof language."
            value={form.seoDescription}
            rows={4}
            onValueChange={(value) => update("seoDescription", value)}
          />
        </FieldGroup>
      </FieldSet>
        ...
        <FieldGroup>
          <div className={dashboardClass.formGrid2}>
            <LabeledInput
              label="CTA label"
              value={form.ctaLabel}
              onValueChange={(value) => update("ctaLabel", value)}
            />
            <LabeledInput
              label="CTA href"
              value={form.ctaHref}
              onValueChange={(value) => update("ctaHref", value)}
            />
          </div>
          <LabeledTextarea
          ...
          <LabeledTextarea
            ...
            rows={7}
            onValueChange={(value) => update("structureNotes", value)}
          />
        </FieldGroup>
      </FieldSet>
...
function ProjectImagesCard({
  ...
  const { previewImage, selectedImage, selectedImageNeedsReview } =
    resolveProjectMediaPreview(project.images);
  const selectedImagePreviewStatus = selectedImage?.previewStatus;
  const selectedImageHasPreviewIssue =
    selectedImagePreviewStatus !== undefined &&
  ...
  return (
    <Card className={dashboardClass.cardShadow}>
      <CardHeader className={dashboardClass.mediaCardHeader}>
        ...
        <CardDescription>
          Choose the exact image that the Astro landing and case-study pages
          should use.
        </CardDescription>
      </CardHeader>
      <CardContent className={dashboardClass.mediaList}>
        <div className={dashboardClass.mediaSelectedPreview}>
          ...
          <div className={dashboardClass.mediaPreviewCopy}>
            <span
              className={
                selectedImage ? dashboardClass.mediaSelectedLabel : undefined
              }
            >
              {selectedImage
                ? "Selected for Astro"
            ...
            <p>
              {selectedImageHasPreviewIssue
                ? (selectedImage?.previewIssue ??
                  "This selected media record cannot resolve to a safe public image, so Astro will keep using another reviewed project image.")
                : selectedImage
        ...
        {project.images.length > 0 ? (
          project.images.map((image) => (
            <div
              ...
              className={cn(
                dashboardClass.mediaRow,
                image.selectedForPublic && dashboardClass.mediaRowSelected,
              )}
            >
              <MediaImage
              ...
              <div className={dashboardClass.mediaRowBody}>
                ...
                <div className={dashboardClass.mediaRowTags}>
                  ...
                  {image.selectedForPublicAt ? (
                    <Badge>Astro image</Badge>
                  ) : null}
                  {image.selectedForPublic && !image.selectedForPublicAt ? (
                ...
                <div className={dashboardClass.mediaRowActions}>
                  {image.id && image.status !== "archived" ? (
                    <Button
                      type="button"
                      variant={
                        image.selectedForPublicAt ? "secondary" : "outline"
                      }
                      size="sm"
                      disabled={
                        savingKey === `${image.id}:select` || !image.src
                      }
                      title={
                        !image.src
                          ? (image.previewIssue ??
                            "This media record has no safe public image URL.")
                          : undefined
                      }
                      onClick={() =>
                        void onSelectMedia({
          ...
          <p className={dashboardClass.mediaEmptyNote}>
            No project media has been attached yet. Add an image below to give
            this case study a public visual.
          </p>
        )}
      </CardContent>
    </Card>
...
function mediaPreviewIssueLabel(
  status: NonNullable<DashboardProject["images"][number]["previewStatus"]>,
): string {
  switch (status) {
...
function MediaImage({
  ...
  if (!src || failedSrc === src) {
    return (
      <div
        className={cn(dashboardClass.mediaImageEmpty, className)}
        role="img"
        aria-label={alt}
      >
        <ImageIcon aria-hidden="true" />
        <small>
          {failedSrc === src && src ? "Image not reachable" : missingLabel}
        </small>
      </div>
    );
  }
...
function ImageUploadForm({
  ...
  const projectDefaultStorageKey = defaultProjectMediaStorageKey(
    project.contentId,
  );
  ...
  const storageKeyValidation = validateCloudflareImagesCustomId(
    form.storageKey,
  );
  ...
  return (
    <Card className={dashboardClass.cardShadow}>
      <CardHeader className={dashboardClass.mediaCardHeader}>
        ...
        <CardDescription>
          Preview the selected file, or register an image that is already hosted
          publicly.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form
          ...
          onSubmit={(event) => {
            ...
            if (trimmedPublicUrl) {
              void onSaveExternal({
                ...form,
                storageKey: storageKeyValidation.isValid
                  ? storageKeyValidation.value
                  : projectDefaultStorageKey,
        ...
        >
          <FieldGroup>
            <Field>
              <FieldLabel htmlFor={`upload-file-${project.contentId}`}>
                Local image
              </FieldLabel>
              <Input
                ...
                onChange={(event) => {
                  ...
                  if (selectedFile) {
                    setForm((current) => ({
                      ...current,
                      storageKey: cloudflareImagesStorageKeyForFile(
                        projectDefaultStorageKey,
                        selectedFile.name,
                      ),
                    }));
            ...
            {previewUrl ? (
              <figure className={dashboardClass.uploadPreview}>
                <img
                  src={previewUrl}
                  alt={form.altText || "Selected image preview"}
                />
                <figcaption>{file?.name}</figcaption>
              </figure>
            ) : null}
            <Field>
              <FieldLabel htmlFor={`upload-url-${project.contentId}`}>
                Public image URL
              </FieldLabel>
              <Input
              ...
              />
              <FieldDescription>
                Use this instead when the image is already hosted publicly. It
                becomes the selected Astro image.
              </FieldDescription>
            </Field>
            <Field>
              <FieldLabel htmlFor={`upload-alt-${project.contentId}`}>
                Alt text
              </FieldLabel>
              <Textarea
            ...
            <div className={dashboardClass.formGrid2}>
              ...
              <Field data-invalid={showStorageKeyError}>
                <FieldLabel htmlFor={`upload-key-${project.contentId}`}>
                  Storage key
                </FieldLabel>
                <Input
...
function projectFormsEqual(
  left: ProjectFormState,
  right: ProjectFormState,
): boolean {
