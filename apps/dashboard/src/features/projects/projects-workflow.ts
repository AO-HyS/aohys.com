...
export async function runProjectMediaUpload(
  ...
  if (!storageKey.isValid)
    throw new MediaUploadValidationError(
      mediaUploadCustomIdIssue(storageKey.message),
    );
...
export function useProjectsWorkflow({
  ...
  const requestProjectPublication = usePublishProject();
  ...
  const [deleteRequest, setDeleteRequest] =
    useState<MediaSelectionRequest | null>(null);
  ...
  async function saveProject(payload: ProjectFormState) {
    ...
    try {
      ...
      captureDashboardAction("succeeded", "projects", "save_project", {
        locale: payload.locale,
      });
      ...
      captureDashboardAction(
        ...
        "projects",
        "save_project",
        errorProperties(error, payload.locale),
      );
      toast.error("Draft save failed", {
        id: toastId,
        description: errorMessage(error, "Project draft could not be saved."),
      });
    } finally {
      setSavingKey(null);
    }
  ...
  async function uploadMedia(payload: MediaUploadRequest, file: File) {
    ...
    const toastId = toast.loading("Uploading project evidence", {
      description: "Preparing the reviewed media reference.",
    });
    setSavingKey(key);
    try {
      ...
      toast.success("Image uploaded", {
        id: toastId,
        description:
          "It is selected for the public Astro build. Publish when ready.",
      });
    } catch (error) {
      captureDashboardAction(
        "failed",
      ...
      const issue =
        error instanceof MediaUploadValidationError
          ? error.issue
          : mediaUploadIssueFromError(error);
  ...
  async function selectMedia(payload: MediaSelectionRequest) {
    await runSimpleOperation({
      ...
      successDescription:
        "This image is the one the Astro build will use for the project.",
  ...
  async function archiveMedia(payload: MediaSelectionRequest) {
    await runSimpleOperation({
      ...
      successDescription:
        "The media record is archived and will not be sent to Astro.",
  ...
  async function createProject(input: NewProjectInput) {
    ...
    if (existingContentIds.includes(contentId)) {
      toast.error("Project content key already exists", {
        description:
          "Choose a different stable content key before creating this draft.",
      });
      return false;
    }
    ...
    try {
      const createdContentId = await runProjectCreation(
        input,
      ...
      toast.success("Project created", {
        id: toastId,
        description:
          "The draft exists in both languages. Save details, attach media, then publish.",
      });
      return true;
    } catch (error) {
      captureDashboardAction(
      ...
      toast.error("Project creation failed", {
        ...
        description: errorMessage(
          error,
          "The project draft could not be created.",
        ),
      });
      return false;
    } finally {
  ...
  async function publishProject(project: DashboardProject) {
    ...
    try {
      const result = await runProjectPublication(project, ({ contentId }) =>
        requestProjectPublication(contentId),
      );
      ...
      toast.error("Publish failed", {
        ...
        description: errorMessage(
          error,
          "Project publish could not be queued.",
        ),
      });
    } finally {
  ...
  async function runSimpleOperation(config: {
    ...
    try {
      ...
      captureDashboardAction(
        ...
        "projects",
        config.action,
        errorProperties(error, config.locale),
      );
      toast.error(config.failure, {
        id: toastId,
        description: errorMessage(error, config.fallbackError),
      });
    } finally {
      setSavingKey(null);
    }
...
export function buildNewProjectDraft(
  input: NewProjectInput,
  locale: DashboardLocale,
): ProjectFormState {
  const contentId = `case-study:${input.contentKey}`;
  const localizedTitle = getLocalizedValue(locale, {
    en: input.title,
    es: input.spanishTitle,
  });
  const localizedSlug = getLocalizedValue(locale, {
    en: input.englishSlug,
    es: input.spanishSlug,
  });
...
function mediaUploadCustomIdIssue(detail?: string): MediaUploadIssue {
  return {
    ...
    description:
      "Cloudflare Images needs a safe relative custom ID before it can create an upload slot.",
    detail:
      detail ??
      "Use a value like media/casa-roca/hero without URLs, dot folders, traversal, or query fragments.",
...
function mediaUploadIssueFromError(error: unknown): MediaUploadIssue {
  return {
    ...
    description:
      "The evidence asset was not registered. No publication state changed.",
    detail: errorMessage(
      error,
      "The provider did not complete the media upload.",
    ),
