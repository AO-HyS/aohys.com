...
import type { CreateProjectRequest } from "./projects-api";
import {
...
describe("Projects workflow", () => {
  it("builds localized draft identity and labels from the shared i18n contract", () => {
    ...
    expect(buildNewProjectDraft(input, "en")).toEqual(
    ...
    expect(buildNewProjectDraft(input, "es")).toEqual(
      expect.objectContaining({
  ...
  it("creates both locale drafts through one atomic request with independent routes", async () => {
    const createProject = vi.fn(
      async (_request: CreateProjectRequest) => undefined,
    );
    ...
    await runProjectMediaUpload(
      ...
      file,
      {
        createUpload: async () => {
          ...
          return {
            imageId: "image-id",
            publicUrl: "https://imagedelivery.net/hash/image-id/public",
