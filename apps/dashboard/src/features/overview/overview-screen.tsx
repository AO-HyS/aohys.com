...
import { useDashboardOverview } from "./overview-api";
...
const stateCopy = {
  clear: {
    ...
    description:
      "No unpublished project or resume work is waiting. Review a source surface when you want to prepare the next release.",
  ...
  "action-required": {
    ...
    description:
      "The readiness contract found work that must be completed before the next publication request can be trusted.",
  ...
  "ready-to-queue": {
    ...
    description:
      "The current drafts satisfy the bounded readiness gates. Open the source surface to review and explicitly queue publication.",
  ...
  partial: {
    ...
    description:
      "The bounded query reached a safety limit, so this workspace will not make a complete publication claim.",
...
const gatePresentation = {
  ...
  OverviewGate["status"],
  { tone: StatusTone; icon: typeof CircleDotIcon }
>;
    ...
    <div className="flex flex-col gap-6">
      ...
      >
        ...
        <TruthCell
          ...
          value={
            overview.release.providerState === "configured"
              ? "Configured"
              : "Unavailable"
          }
          tone={
        ...
        >
          <ol className="divide-y" aria-label="Publication readiness gates">
            {overview.gates.map((gate) => (
              <ReadinessGate key={gate.id} gate={gate} />
            ))}
          </ol>
        </SectionPanel>
        ...
        <div className="flex flex-col gap-5">
          ...
          >
            {overview.nextAction ? (
              ...
              <p className="font-body leading-6 text-muted-foreground">
                There is no corrective dashboard action right now. Start the
                next draft from its source surface.
              </p>
            )}
          </SectionPanel>
          ...
          <Alert
            className="p-4"
            variant={
              overview.release.providerState === "unavailable"
                ? "destructive"
                : "default"
            }
          >
            ...
            <AlertDescription className="font-body leading-6">
              Ready to queue is not workflow queued. Workflow queued is not
              deployment verified. This surface reports the latter as unknown
              until authoritative receipts exist.
            </AlertDescription>
          </Alert>
        </div>
      </div>
    ...
    <li className="grid gap-3 py-4 first:pt-0 last:pb-0 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-start">
      <div className="flex min-w-0 gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-1 max-w-[68ch] font-body text-sm leading-6 text-muted-foreground">
            {gate.reason}
          </p>
        </div>
      </div>
