...
export function ResumeScreen() {
  ...
  const baseline = useMemo(() => {
    ...
    const draft = payload.resumeDrafts.find(
...
function ResumeWorkspace({
  ...
  const [seenBaselineVersion, setSeenBaselineVersion] =
    useState(baselineVersion);
  const [isSaving, setIsSaving] = useState(false);
  const [isSavingVersion, setIsSavingVersion] = useState(false);
  const [isPublishing, setIsPublishing] = useState(false);
  const [saveResult, setSaveResult] = useState<"clean" | "saved" | "error">(
    "clean",
  );
  ...
  async function handleSave() {
    if (validationErrors.length > 0) {
      setSaveResult("error");
      toast.error("Resume needs attention", {
    ...
    try {
      ...
      setSaveResult("saved");
      captureDashboardAction("succeeded", "resume", "save_resume", {
        locale: selectedLocale,
      });
      toast.success("Resume saved", {
        id: toastId,
        description:
          "The editable resume draft is saved. Publish to rebuild the public Astro resume.",
      });
    } catch (saveError) {
      setSaveResult("error");
      ...
      toast.error("Resume save failed", {
        ...
        description:
          saveError instanceof Error
            ? saveError.message
            : "Resume draft could not be saved.",
      });
    } finally {
      setIsSaving(false);
    }
  ...
  async function handlePublish() {
    ...
    try {
      const result = await publishResume(selectedLocale);
      ...
      toast.error("Resume publish failed", {
        ...
        description:
          publishError instanceof Error
            ...
            : "Resume publish could not be queued.",
      });
    } finally {
  ...
  async function handleVersionSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      toast.success("PDF artifact saved", {
        id: toastId,
        description:
          "The PDF artifact is registered. Resume copy is edited in the main editor above.",
      });
    } catch (saveError) {
      ...
      toast.error("PDF artifact failed", {
        ...
        description:
          saveError instanceof Error
            ? saveError.message
            : "Resume version could not be saved.",
      });
    } finally {
...
function ResumeWorkspaceStatus({
  ...
  return (
    <>
      ...
      <section
        aria-label="Resume draft status"
        className="flex flex-wrap gap-x-6 gap-y-2 border-y py-3 text-sm text-muted-foreground"
      >
        <span>
          Last saved:{" "}
        ...
        <span>
          Last published:{" "}
          <strong className="text-foreground">
            {activeDraft?.publishedAt
              ? formatDate(activeDraft.publishedAt)
              : "Not published"}
          </strong>
        </span>
        <span>
      ...
      {validationErrors.length > 0 ? (
        <div
          role="alert"
          className="rounded-lg border border-attention/40 bg-attention/10 p-4"
        >
      ...
      {hasRemoteBaseline ? (
        <div
          role="status"
          className="flex flex-col gap-3 rounded-lg border border-attention/40 bg-attention/10 p-4 sm:flex-row sm:items-center sm:justify-between"
        >
          <div>
            <strong className="text-sm">
              A newer saved baseline is available
            </strong>
            <p className="mt-1 font-body text-sm text-muted-foreground">
              Rebase only when you are ready to replace this local editor state.
            </p>
          </div>
          <ConfirmAction
...
function ResumeEditor({
  ...
  function update<K extends keyof DashboardResumeContent>(
    key: K,
    value: DashboardResumeContent[K],
  ) {
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0">
        {section === "header" ? (
          <ResumeHeaderEditor content={content} update={update} />
        ) : null}
        {section === "proof" ? (
          <ResumeProofEditor content={content} update={update} />
        ) : null}
        {section === "summary" ? (
          <TextArrayEditor
...
function ResumeSectionSelect({
  ...
  return (
    <LabeledSelect
      ...
      options={resumeSections.map((item) => ({
        value: item.id,
        label: item.label,
      }))}
    />
...
function ResumeHeaderEditor({
  ...
  content: DashboardResumeContent;
  update: <K extends keyof DashboardResumeContent>(
    key: K,
    value: DashboardResumeContent[K],
  ) => void;
}) {
  return (
    <Card>
      <CardHeader>
        <CardTitle>Header & download CTA</CardTitle>
        <CardDescription>
          Name, positioning, intro, and public PDF action.
        </CardDescription>
      </CardHeader>
      <CardContent className={dashboardClass.editForm}>
        ...
        <LabeledTextarea
          ...
          value={content.intro}
          rows={4}
          onValueChange={(value) => update("intro", value)}
        />
        <div className={dashboardClass.formGrid2}>
          <LabeledInput
            label="PDF label"
            value={content.pdf.label}
            onValueChange={(value) =>
              update("pdf", { ...content.pdf, label: value })
            }
          />
          <LabeledInput
            label="PDF href"
            value={content.pdf.href}
            onValueChange={(value) =>
              update("pdf", { ...content.pdf, href: value })
            }
          />
        </div>
      </CardContent>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          The evidence context beside the resume hero.
        </CardDescription>
      </CardHeader>
      <CardContent className={dashboardClass.editForm}>
        <LabeledInput
          label="Proof label"
          value={content.proof.label}
          onValueChange={(value) =>
            update("proof", { ...content.proof, label: value })
          }
        />
        <LabeledInput
          label="Proof title"
          ...
          onValueChange={(value) =>
            update("proof", { ...content.proof, title: value })
          }
        />
        <LabeledTextarea
          label="Proof body"
          value={content.proof.body}
          rows={4}
          onValueChange={(value) =>
            update("proof", { ...content.proof, body: value })
          }
        />
      </CardContent>
    </Card>
          ...
          <div key={itemKeys.at(index)} className={dashboardClass.arrayRow}>
            <LabeledTextarea
              ...
              onValueChange={(nextValue) =>
                onChange(replaceAt(values, index, nextValue))
              }
            />
            <IconButton
              label="Remove"
          ...
          >
            <div className={dashboardClass.formGrid2}>
              <LabeledInput
                ...
                value={item.label}
                onValueChange={(value) =>
                  onChange(replaceAt(items, index, { ...item, label: value }))
                }
              />
              <LabeledInput
                label={`Highlight ${index + 1} text`}
                value={item.text}
                onValueChange={(value) =>
                  onChange(replaceAt(items, index, { ...item, text: value }))
                }
              />
            </div>
            <IconButton
        ...
        {items.map((item, index) => (
          ...
          >
            <LabeledInput
              ...
              value={item.title}
              onValueChange={(value) =>
                onChange(replaceAt(items, index, { ...item, title: value }))
              }
            />
            <LabeledTextarea
              label={`Project ${index + 1} summary`}
              value={item.summary}
              rows={3}
              onValueChange={(value) =>
                onChange(replaceAt(items, index, { ...item, summary: value }))
              }
            />
            <LabeledTextarea
              label={`Project ${index + 1} bullets`}
              ...
              onValueChange={(value) =>
                onChange(
                  replaceAt(items, index, {
                    ...item,
                    bullets: linesFromTextarea(value),
                  }),
                )
              }
            />
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{title}</CardTitle>
        <CardDescription>
          Add and edit roles, periods, and impact bullets.
        </CardDescription>
      </CardHeader>
      <CardContent className={dashboardClass.editForm}>
        <LabeledInput
          ...
          >
            <div className={dashboardClass.formGrid3}>
              <LabeledInput
                ...
                value={item.role}
                onValueChange={(value) =>
                  onChange(replaceAt(items, index, { ...item, role: value }))
                }
              />
              <LabeledInput
                ...
                value={item.company}
                onValueChange={(value) =>
                  onChange(replaceAt(items, index, { ...item, company: value }))
                }
              />
              <LabeledInput
                label={`Experience ${index + 1} period`}
                value={item.period}
                onValueChange={(value) =>
                  onChange(replaceAt(items, index, { ...item, period: value }))
                }
              />
            </div>
            <LabeledTextarea
              label={`Experience ${index + 1} bullets`}
              description="One bullet per line."
              value={item.bullets.join("\n")}
              rows={5}
              onValueChange={(value) =>
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{title}</CardTitle>
        <CardDescription>
          Skill groups and comma-free item lists.
        </CardDescription>
      </CardHeader>
      <CardContent className={dashboardClass.editForm}>
        <LabeledInput
          ...
          >
            <div className={dashboardClass.formGrid3}>
              <LabeledInput
                ...
                value={item.degree}
                onValueChange={(value) =>
                  onChange(replaceAt(items, index, { ...item, degree: value }))
                }
              />
              <LabeledInput
                ...
                onValueChange={(value) =>
                  onChange(
                    replaceAt(items, index, { ...item, institution: value }),
                  )
                }
              />
              <LabeledInput
                label={`Education ${index + 1} period`}
                value={item.period}
                onValueChange={(value) =>
                  onChange(replaceAt(items, index, { ...item, period: value }))
                }
...
function ResumeVersionsCard({
  ...
  versions: DashboardResumeVersion[] | null;
}) {
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Downloadable files registered for the public resume.
        </CardDescription>
      </CardHeader>
      <CardContent>
        {!versions ? (
