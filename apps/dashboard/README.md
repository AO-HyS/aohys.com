...
The browser receives only the dashboard runtime config needed to connect to Convex: `convexUrl`, `betterAuthUrl`, environment, admin email, and the Cloudflare Images delivery hash. React reads and writes through admin-gated public Convex functions with Better Auth JWTs; the Pages Function no longer proxies dashboard JSON requests.
