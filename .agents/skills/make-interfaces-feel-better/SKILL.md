...
description: Refine UI craft when visual or interaction details feel off, including motion, alignment, borders, and shadows.
---
...
Assign visual judgment and Computer Use by actual capability within the user's
provider limits. Preserve the product's tokens,
typography, density, brand and interaction conventions. Numeric style values
below are examples, not required replacements for a coherent design system.
Use the quick path for a bounded interaction and a full review for an explicitly
broad design task. Respect reduced motion. Polish suggestions are not automatic
release gates.
