...
const assessmentTemplatePath = resolve(
  skillDirectory,
  "assets",
...
export async function findSessionFile(sessionsRoot, threadId) {
  ...
  throw new Error(
    `Cannot find the Codex session for thread ${threadId} under ${root}`,
  );
}

/** Discover direct children from headers only; never scan unrelated transcripts.
 * @param {string} root @param {string} parentId @param {number} cutoffMs
 */
async function discoverChildHeaders(root, parentId, cutoffMs) {
  const children = [];
  const directories = [root];
      ...
      try {
        for await (const line of lines) {
          ...
          if (
            ...
            timestampMs(header.timestamp) <= cutoffMs
          ) {
            children.push({
              threadId: payload.id,
        ...
        /* An incomplete header is unavailable, not evidence of execution. */
      } finally {
        lines.close();
        stream.destroy();
      }
...
function customOutputStatus(output) {
  ...
  if (
    /^(script failed|process exited with code [1-9]|command failed|tool error|timed out)/i.test(
      header.trim(),
    )
  ) {
...
const taskStartTypes = new Set(["task_started", "turn_started"]);
const taskEndTypes = new Set([
  "task_complete",
  "task_completed",
  "turn_complete",
  "turn_completed",
...
/** @param {unknown} type */
function isTaskStartType(type) {
  return typeof type === "string" && taskStartTypes.has(type);
}

/** @param {unknown} type */
function isTaskEndType(type) {
  return typeof type === "string" && taskEndTypes.has(type);
}

/** @param {unknown} value @param {Set<string>} seen @param {string[]} found */
function collectUuids(value, seen, found) {
  if (typeof value === "string") {
    const matches = value.match(
      /[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[89ab][0-9a-f]{3}-[0-9a-f]{12}/gi,
    );
    if (matches) {
      for (const match of matches) {
        const normalized = match.toLowerCase();
        if (uuidPattern.test(normalized) && !seen.has(normalized)) {
          seen.add(normalized);
  ...
  if (Array.isArray(value)) {
    for (const item of value) collectUuids(item, seen, found);
    return;
  }
  if (isRecord(value)) {
    for (const [key, entry] of Object.entries(value)) {
      if (/agent|thread|child|task|spawn/i.test(key))
        collectUuids(entry, seen, found);
    }
    // Fall back to nested scan only through likely containers to avoid false positives.
    for (const [key, entry] of Object.entries(value)) {
      if (!/agent|thread|child|task|spawn|output|result|data/i.test(key))
        continue;
...
/** @param {unknown} output */
function extractSpawnChildIds(output) {
  /** @type {string[]} */
  const found = [];
  const seen = new Set();
  if (typeof output === "string") {
    ...
    if (trimmed.startsWith("{") || trimmed.startsWith("[")) {
      try {
        collectUuids(JSON.parse(trimmed), seen, found);
      } catch {
        collectUuids(trimmed, seen, found);
      }
    } else {
      collectUuids(trimmed, seen, found);
    }
  ...
  if (Array.isArray(output)) {
    for (const item of output) {
      if (isRecord(item) && typeof item.text === "string")
        collectUuids(item.text, seen, found);
      else collectUuids(item, seen, found);
    }
  } else {
    collectUuids(output, seen, found);
  }
  return found;
}

/**
 * Sum a cumulative counter across resets (e.g. token totals that restart at
 * zero on resume). Repeated identical snapshots contribute zero.
 * @param {number[]} values ordered ascending by time
 */
function sumCumulativeWithResets(values) {
  const numeric = values.filter(
    (value) => typeof value === "number" && Number.isFinite(value),
  );
  if (numeric.length === 0) return null;
  let total = numeric[0] ?? 0;
  let previous = numeric[0] ?? 0;
  for (let index = 1; index < numeric.length; index += 1) {
    const current = /** @type {number} */ (numeric[index]);
...
/**
 * Build bounded operational intervals: each start is clipped at the next
 * start, at a matching (or turn-less) terminator, and at the cutoff.
 * @param {Array<{timestamp: string, kind: string, turnId?: string, durationMs?: number}>} taskEvents
 * @param {number} cutoffMs
 */
function buildBoundedIntervals(taskEvents, cutoffMs) {
  const starts = taskEvents
    .filter(
      (event) => isTaskStartType(event.kind) || event.kind === "task_started",
    )
    .map((event) => ({
      time: timestampMs(event.timestamp),
      turnId: event.turnId ?? null,
      event,
    }))
    .filter((entry) => entry.time <= cutoffMs)
    .sort((left, right) => left.time - right.time);
  const ends = taskEvents
    .filter(
      (event) => isTaskEndType(event.kind) || event.kind === "task_complete",
    )
    .map((event) => ({
      time: timestampMs(event.timestamp),
      turnId: event.turnId ?? null,
  ...
  for (const end of ends) {
    if (!end.turnId) continue;
    const existing = byTurn.get(end.turnId);
    if (!existing || end.time < existing.time) byTurn.set(end.turnId, end);
  }
  ...
  for (let index = 0; index < starts.length; index += 1) {
    const start =
      /** @type {{time: number, turnId: string | null, event: any}} */ (
        starts[index]
      );
    const nextStartTime =
      index + 1 < starts.length
        ? /** @type {{time: number}} */ (starts[index + 1]).time
        : cutoffMs;
    let endTime = Math.min(nextStartTime, cutoffMs);
    let matchedEnd = null;
    if (start.turnId) {
      const candidate = byTurn.get(start.turnId);
      if (
        candidate &&
        candidate.time >= start.time &&
        candidate.time <= endTime
      )
        matchedEnd = candidate;
    }
    if (!matchedEnd) {
      const globalEnd = ends.find(
        (end) =>
          end.time >= start.time &&
          end.time <= endTime &&
          (!end.turnId || !start.turnId || end.turnId === start.turnId),
      );
      if (globalEnd) matchedEnd = globalEnd;
    }
    if (matchedEnd) endTime = matchedEnd.time;
    if (endTime <= start.time) continue;
    // Prefer the reported duration for completed turns (compatibility), but
    // never extend past the bounded end.
    if (
      matchedEnd &&
      typeof matchedEnd.event.durationMs === "number" &&
      Number.isFinite(matchedEnd.event.durationMs)
    ) {
      const reportedStart = Math.max(
        start.time,
        matchedEnd.time - Math.max(0, matchedEnd.event.durationMs),
      );
      const interval = {
        startedAt: new Date(
          Math.min(reportedStart, matchedEnd.time),
        ).toISOString(),
        endedAt: new Date(matchedEnd.time).toISOString(),
      };
      // Reported durations cannot extend before the observed task start.
      if (timestampMs(interval.startedAt) < endTime) {
    ...
    intervals.push(interval);
    if (matchedEnd) completedIntervals.push(interval);
    else if (index === starts.length - 1) openIntervals.push(interval);
  }
  return { intervals, openIntervals, completedIntervals };
}
...
export async function readSessionMetadata(sessionPath) {
  ...
  const agentSpawns = [];
  /** @type {Array<{timestamp: string, threadId: string, callId: string}>} */
  const spawnChildren = [];
  ...
  for await (const line of lines) {
    ...
    if (record.type === "event_msg" && isRecord(record.payload)) {
      ...
      if (isTaskStartType(type) || isTaskEndType(type)) {
        // Error/interruption terminators without a turn still bound the open
        // turn; generic errors lacking any turn context are ignored.
        if (
          (type === "error" || type === "interruption") &&
          !asString(record.payload.turn_id)
        )
          continue;
        taskEvents.push({
          timestamp,
          ...
          timeToFirstTokenMs:
            asNumber(record.payload.time_to_first_token_ms) ?? undefined,
        });
      } else if (type === "token_count" && isRecord(record.payload.info)) {
        tokenEvents.push({
          timestamp,
          usage: record.payload.info.total_token_usage ?? null,
        });
    ...
    if (payloadType === "message") {
      ...
      if (role === "user" || role === "assistant")
        messages.push({ timestamp, role, source: "response-item" });
      continue;
    }
    ...
    if (
      ...
      if (call && call.name === "spawn_agent" && !status.failed) {
        for (const childId of extractSpawnChildIds(record.payload.output)) {
          if (childId !== session?.id)
            spawnChildren.push({ timestamp, threadId: childId, callId });
        }
      }
    }
  }
  if (!session?.id)
    throw new Error(`Session metadata is missing from ${sessionPath}`);
  return {
    session,
    messages,
    taskEvents,
...
function intervalsOverlap(left, right) {
  return (
    timestampMs(left.startedAt) < timestampMs(right.endedAt) &&
...
function resolveCutoff(metadata, cutoffOption) {
  ...
  if (cutoffOption !== "latest-user") {
    ...
    return {
      value: new Date(cutoffOption).toISOString(),
      mode: "explicit-timestamp",
    };
  }
  const latest = metadata.messages
    ...
    .at(-1);
  if (!latest)
    throw new Error("Cannot locate a user prompt for the invocation cutoff");
...
function uniqueMessages(messages) {
  const hasEventUsers = messages.some(
    (message) => message.role === "user" && message.source === "event",
  );
  const hasResponseAssistants = messages.some(
    (message) =>
      message.role === "assistant" && message.source === "response-item",
  );
  const preferred = messages.filter((message) => {
    if (message.role === "user")
      return hasEventUsers
        ? message.source === "event"
        : message.source === "response-item";
    return hasResponseAssistants
      ? message.source === "response-item"
      : message.source === "event";
  });
  const seen = new Set();
...
function summarizeAgents(events) {
  ...
  for (const event of ordered) {
    ...
    if (event.kind === "started") {
      ...
      ["completed", "failed", "errored", "interrupted", "closed"].includes(
        event.kind,
      )
    ) {
      ...
      if (start !== undefined) {
        durationByThread.set(
          event.threadId,
          (durationByThread.get(event.threadId) ?? 0) +
            Math.max(0, timestampMs(event.timestamp) - start),
        );
  ...
  return {
    childThreadIds: [
      ...new Set(ordered.map((event) => event.threadId).filter(Boolean)),
    ],
    peakChildren,
...
/** @param {string} sessionsRoot @param {string[]} childThreadIds @param {number} cutoffMs @param {Map<string, number>} [spawnTimes] */
async function childAgentDetails(
  sessionsRoot,
  childThreadIds,
  cutoffMs,
  spawnTimes = new Map(),
  ...
  for (const threadId of childThreadIds) {
    try {
      ...
      // Exclude inherited parent history: only count child work at/after the
      // evidenced spawn time. Without a spawn time the window is unknown, so
      // fall back to the full session but mark it honestly below.
      const lowerBoundMs = spawnTimes.get(threadId);
      const inWindow = (timestamp) => {
        const time = timestampMs(timestamp);
        if (time > cutoffMs) return false;
        if (lowerBoundMs !== undefined && time < lowerBoundMs) return false;
        return true;
      };
      const context = metadata.contexts
        .filter(
          (item) =>
            inWindow(item.timestamp) && timestampMs(item.timestamp) <= cutoffMs,
        )
        .sort(
          (left, right) =>
            timestampMs(left.timestamp) - timestampMs(right.timestamp),
        ...
        .at(-1);
      const taskEvents = metadata.taskEvents.filter((event) =>
        inWindow(event.timestamp),
      );
      const bounded = buildBoundedIntervals(taskEvents, cutoffMs);
      // Clip bounded intervals below the spawn lower bound (reported
      // durations may otherwise reach back into inherited history).
      const ownedIntervals =
        lowerBoundMs === undefined
          ? bounded.intervals
          : bounded.intervals
              .filter(
                (interval) => timestampMs(interval.endedAt) > lowerBoundMs,
              )
              .map((interval) => ({
                startedAt: new Date(
                  Math.max(
                    timestampMs(interval.startedAt),
                    /** @type {number} */ (lowerBoundMs),
                  ),
                ).toISOString(),
              ...
              .filter(
                (interval) =>
                  timestampMs(interval.endedAt) >
                  timestampMs(interval.startedAt),
              );
      const durationMs =
        ownedIntervals.length > 0 ? unionDurationMs(ownedIntervals) : null;
      const startedAt =
        ownedIntervals
          ...
          .at(0) ?? null;
      const hasOpen =
        bounded.openIntervals.length > 0 &&
        (lowerBoundMs === undefined ||
          bounded.openIntervals.some(
            (interval) => timestampMs(interval.endedAt) > lowerBoundMs,
          ));
      const endedAt = hasOpen
        ? null
        : (ownedIntervals
            .map((interval) => interval.endedAt)
            .sort()
            .at(-1) ?? null);
      const orderedTokens = metadata.tokenEvents
        .filter((event) => inWindow(event.timestamp))
        .sort(
          (left, right) =>
            timestampMs(left.timestamp) - timestampMs(right.timestamp),
        );
      const previousToken = metadata.tokenEvents
        .filter(
          (event) =>
            lowerBoundMs !== undefined &&
            timestampMs(event.timestamp) < lowerBoundMs,
        )
        .sort(
          (left, right) =>
            timestampMs(left.timestamp) - timestampMs(right.timestamp),
        ...
        .at(-1);
      const baseline = isRecord(previousToken?.usage)
        ? asNumber(previousToken.usage.total_tokens)
        : null;
      const values = orderedTokens
        .map((event) =>
          isRecord(event.usage) ? asNumber(event.usage.total_tokens) : null,
        )
        .filter((value) => typeof value === "number");
      const childTokensTotal =
        lowerBoundMs === undefined || values.length === 0
          ? null
          : (sumCumulativeWithResets(
              baseline === null ? values : [baseline, ...values],
            ) ?? 0) - (baseline ?? 0);
      const completedOutputs = new Map(
        metadata.outputs
          .filter((output) => inWindow(output.timestamp))
          .map((output) => [output.callId, output]),
      );
      const childCalls = [...metadata.calls.entries()].filter(([, call]) =>
        inWindow(call.timestamp),
      );
      details.push({
        threadId,
        ...
        telemetryStatus:
          lowerBoundMs === undefined
            ? "observed-unbounded: no evidenced spawn time; inherited history cannot be excluded"
            : "observed",
        executionStatus: hasOpen
          ? "in-progress"
          : endedAt
            ? "completed"
            : "unavailable",
        startedAt,
        endedAt,
        durationMs: lowerBoundMs === undefined ? null : durationMs,
        tokens: childTokensTotal,
        toolCalls: childCalls.length,
        toolFailures: childCalls.filter(
          ([callId]) => completedOutputs.get(callId)?.failed === true,
        ).length,
      });
    } catch (error) {
      details.push({
...
export async function collectTelemetry(options) {
  ...
  if (!firstUser)
    throw new Error(
      "Cannot locate the first user prompt in the selected interval",
    );
  const startedAtMs = timestampMs(firstUser.timestamp);
  if (cutoffMs < startedAtMs)
    throw new Error("Cutoff precedes the first user prompt");
  ...
  const taskEvents = metadata.taskEvents.filter(
    (event) => timestampMs(event.timestamp) <= cutoffMs,
  );
  const completedTurns = taskEvents.filter(
    (event) =>
  ...
  const endedTurnIds = new Set(
    taskEvents
      .filter((event) => isTaskEndType(event.kind))
      .map((event) => event.turnId)
      .filter(Boolean),
  );
  const openTurns = taskEvents.filter(
    (event) =>
      isTaskStartType(event.kind) &&
      event.turnId &&
      !endedTurnIds.has(event.turnId),
  );
  const bounded = buildBoundedIntervals(taskEvents, cutoffMs);
  const completedParentTurnMs = unionDurationMs(bounded.completedIntervals);
  const operationalIntervals = bounded.intervals;
  const parentActiveMs =
    operationalIntervals.length > 0 ? unionDurationMs(operationalIntervals) : 0;
  const inProgressParentTurnMs =
    bounded.openIntervals.length > 0
      ? unionDurationMs(bounded.openIntervals)
      : 0;
  const firstTokenMs = completedTurns.reduce(
    (total, event) => total + (event.timeToFirstTokenMs ?? 0),
    0,
  );
  const wallMs = Math.max(0, cutoffMs - startedAtMs);
  const betweenTurnMs = Math.max(0, wallMs - parentActiveMs);

  const orderedTokenEvents = metadata.tokenEvents
    .filter((event) => timestampMs(event.timestamp) <= cutoffMs)
    .sort(
      (left, right) =>
        timestampMs(left.timestamp) - timestampMs(right.timestamp),
    );
  const tokenEvent = orderedTokenEvents.at(-1);
  const usage = isRecord(tokenEvent?.usage) ? tokenEvent.usage : null;
  /** @param {string} field */
  const summedTokenField = (field) =>
    sumCumulativeWithResets(
      orderedTokenEvents
        .map((event) =>
          isRecord(event.usage) ? asNumber(event.usage[field]) : null,
        )
        .filter((value) => typeof value === "number"),
    );
  ...
  for (const [callId, call] of metadata.calls) {
    ...
    if (!output) {
      ...
      const duration = Math.max(
        0,
        timestampMs(output.timestamp) - timestampMs(call.timestamp),
      );
      summary.completed += 1;
  ...
  const toolSummaries = [...tools.values()].sort((left, right) =>
    left.name.localeCompare(right.name),
  );
  const toolTimeMs = toolSummaries.reduce(
    (total, tool) => total + tool.totalDurationMs,
    0,
  );
  const toolFailures = toolSummaries.reduce(
    (total, tool) => total + tool.failed,
    0,
  );
  const incompleteTools = toolSummaries.reduce(
    (total, tool) => total + tool.incomplete,
    0,
  );

  const agentEvents = metadata.agentEvents.filter(
    (event) => timestampMs(event.timestamp) <= cutoffMs,
  );
  const agents = summarizeAgents(agentEvents);
  // Recover modern children from successful spawn_agent outputs even when no
  // sub_agent_activity was emitted. spawnTimes bound child work so inherited
  // parent history is excluded; no T3 DB scan is performed.
  const spawnTimes = new Map();
  const headerChildren =
    metadata.agentSpawns.length > 0
      ? await discoverChildHeaders(
          options.sessionsRoot,
          metadata.session.id,
          cutoffMs,
        )
      : [];
  for (const spawn of [...headerChildren, ...(metadata.spawnChildren ?? [])]) {
    if (timestampMs(spawn.timestamp) > cutoffMs) continue;
    if (!spawn.threadId || spawn.threadId === metadata.session.id) continue;
    if (
      !spawnTimes.has(spawn.threadId) ||
      timestampMs(spawn.timestamp) <
        (spawnTimes.get(spawn.threadId) ?? Infinity)
    ) {
      spawnTimes.set(spawn.threadId, timestampMs(spawn.timestamp));
    }
  }
  const childThreadIds = [
    ...new Set([...agents.childThreadIds, ...spawnTimes.keys()]),
  ];
  const children = await childAgentDetails(
    options.sessionsRoot,
    childThreadIds,
    cutoffMs,
    spawnTimes,
  ...
  const spawnByTask = new Map(
    metadata.agentSpawns
      .filter(
        (spawn) => timestampMs(spawn.timestamp) <= cutoffMs && spawn.taskName,
      )
  ...
  const childActiveMs = children.reduce(
    (total, child) =>
      total + (typeof child.durationMs === "number" ? child.durationMs : 0),
    0,
  );
  const childTokens = children.reduce(
    (total, child) =>
      total + (typeof child.tokens === "number" ? child.tokens : 0),
    0,
  );
  const childToolCalls = children.reduce(
    (total, child) =>
      total + (typeof child.toolCalls === "number" ? child.toolCalls : 0),
    0,
  );
  const childToolFailures = children.reduce(
    (total, child) =>
      total + (typeof child.toolFailures === "number" ? child.toolFailures : 0),
    0,
  );
  const childTimeline = children
    .flatMap((child) => {
      if (!child.startedAt) return [];
      return [
        { timestamp: timestampMs(child.startedAt), delta: 1 },
        ...(child.endedAt
          ? [{ timestamp: timestampMs(child.endedAt), delta: -1 }]
          : []),
      ];
    })
    .sort(
      (left, right) =>
        left.timestamp - right.timestamp || left.delta - right.delta,
    );
  ...
  return {
    ...
    timing: {
      ...
      additiveWarning:
        "toolCallMs and childAgentMs overlap operationalMs; agentCapacityMs measures consumed capacity, not elapsed delivery time",
    },
    tokens:
      orderedTokenEvents.length > 0
        ? {
            input: summedTokenField("input_tokens"),
            cachedInput: summedTokenField("cached_input_tokens"),
            cacheWriteInput: summedTokenField("cache_write_input_tokens"),
            output: summedTokenField("output_tokens"),
            reasoningOutput: summedTokenField("reasoning_output_tokens"),
            total: summedTokenField("total_tokens"),
            childAgentsTotal: childTokens,
            allAgentsTotal:
              (summedTokenField("total_tokens") ?? 0) + childTokens,
            observedAt: tokenEvent?.timestamp ?? null,
            status:
              "summed-nonnegative-increments-across-resets; child sessions added separately",
          }
        : {
            input: null,
            cachedInput: null,
    ...
    messages: {
      user: observedMessages.filter((message) => message.role === "user")
        .length,
      assistant: observedMessages.filter(
        (message) => message.role === "assistant",
      ).length,
    },
    turns: {
      started: taskEvents.filter((event) => isTaskStartType(event.kind)).length,
      completed: completedTurns.length,
      inProgress: openTurns.length,
      abortedOrInterrupted: taskEvents.filter(
        (event) =>
          isTaskEndType(event.kind) &&
          event.kind !== "task_complete" &&
          event.kind !== "turn_complete" &&
          event.kind !== "task_completed" &&
          event.kind !== "turn_completed",
    ...
    tools: {
      ...
      completed: toolSummaries.reduce(
        (total, tool) => total + tool.completed,
        0,
      ),
      ...
      allAgentCalls:
        toolSummaries.reduce((total, tool) => total + tool.calls, 0) +
        childToolCalls,
      allAgentFailures: toolFailures + childToolFailures,
      byName: toolSummaries,
    },
    agents: {
      unique: 1 + childThreadIds.length,
      childAgents: childThreadIds.length,
...
export function validateAssessment(assessment) {
  /** @type {string[]} */
  const errors = [];
  if (!isRecord(assessment) || assessment.schemaVersion !== 2)
    return ["assessment.schemaVersion must equal 2"];
  requireString(assessment.title, "assessment.title", errors);
  if (!workTypes.has(assessment.workType))
    errors.push("assessment.workType is invalid");
  if (!overallStatuses.has(assessment.status))
    errors.push("assessment.status is invalid");
  if (
    assessment.supersedes !== null &&
    !uuidPattern.test(assessment.supersedes ?? "")
  ) {
    errors.push("assessment.supersedes must be null or a measurement UUID");
  }
  if (!isRecord(assessment.objective))
    errors.push("assessment.objective is required");
  else {
    requireString(
  ...
  if (!Array.isArray(assessment.scope) || assessment.scope.length === 0) {
    ...
    assessment.scope.forEach((item, index) => {
      ...
      if (!["initial", "added"].includes(item?.origin))
        errors.push(`assessment.scope[${index}].origin is invalid`);
      if (!scopeStatuses.has(item?.status))
        errors.push(`assessment.scope[${index}].status is invalid`);
      requireStringArray(
        item?.evidence,
        `assessment.scope[${index}].evidence`,
        errors,
      );
    });
  }
  for (const key of [
    ...
    if (!isRecord(entry) || !deliveryStatuses.has(entry.status))
      errors.push(`assessment.outcome.${key}.status is invalid`);
    requireStringArray(
      entry?.evidence,
      `assessment.outcome.${key}.evidence`,
      errors,
    );
  }
  for (const key of ["code", "architecture", "security"]) {
    const entry = assessment.quality?.[key];
    if (!isRecord(entry) || !qualityStatuses.has(entry.status))
      errors.push(`assessment.quality.${key}.status is invalid`);
    requireStringArray(
      entry?.evidence,
      `assessment.quality.${key}.evidence`,
      errors,
    );
    requireStringArray(entry?.gaps, `assessment.quality.${key}.gaps`, errors);
  }
  if (!Array.isArray(assessment.errors))
    errors.push("assessment.errors must be an array");
  else {
    assessment.errors.forEach((entry, index) => {
      if (!errorCategories.has(entry?.category))
        errors.push(`assessment.errors[${index}].category is invalid`);
      requireString(
        entry?.summary,
        `assessment.errors[${index}].summary`,
        errors,
      );
      requireString(
        entry?.impact,
        `assessment.errors[${index}].impact`,
        errors,
      );
      if (typeof entry?.corrected !== "boolean")
        errors.push(`assessment.errors[${index}].corrected must be boolean`);
      requireStringArray(
        entry?.evidence,
        `assessment.errors[${index}].evidence`,
        errors,
      );
    });
  }
  if (!Array.isArray(assessment.externalTiming))
    errors.push("assessment.externalTiming must be an array");
  else {
    assessment.externalTiming.forEach((entry, index) => {
      if (
        !["ci", "deployment", "queue", "provider", "other"].includes(
          entry?.kind,
        )
      ) {
      ...
      requireString(
        entry?.label,
        `assessment.externalTiming[${index}].label`,
        errors,
      );
      if (
        ...
        errors.push(
          `assessment.externalTiming[${index}].durationMs must be a non-negative number`,
        );
      }
      requireString(
        entry?.status,
        `assessment.externalTiming[${index}].status`,
        errors,
      );
      if (
        !["inside-active-turn", "outside-active-turn", "unknown"].includes(
          entry?.overlap,
        )
      ) {
      ...
      requireStringArray(
        entry?.evidence,
        `assessment.externalTiming[${index}].evidence`,
        errors,
      );
      if (typeof entry?.verified !== "boolean") {
        errors.push(
          `assessment.externalTiming[${index}].verified must be boolean`,
        );
      }
      if (
        entry?.verified === true &&
        ...
        errors.push(
          `assessment.externalTiming[${index}].verified timing requires evidence`,
        );
      }
      if (entry?.overlap === "outside-active-turn") {
        ...
        if (
          ...
          errors.push(
            `assessment.externalTiming[${index}] outside-active-turn timing needs a valid interval`,
          );
        } else if (entry?.durationMs !== endedAtMs - startedAtMs) {
          errors.push(
            `assessment.externalTiming[${index}].durationMs must match its interval`,
          );
        }
      }
    });
  }
  if (!Array.isArray(assessment.retries))
    errors.push("assessment.retries must be an array");
  else {
    assessment.retries.forEach((entry, index) => {
      requireString(entry?.kind, `assessment.retries[${index}].kind`, errors);
      if (!Number.isInteger(entry?.count) || entry.count < 1) {
        errors.push(
          `assessment.retries[${index}].count must be a positive integer`,
        );
      }
      requireString(
        entry?.reason,
        `assessment.retries[${index}].reason`,
        errors,
      );
      requireStringArray(
        entry?.evidence,
        `assessment.retries[${index}].evidence`,
        errors,
      );
    });
  }
  for (const key of [
    ...
    if (!Number.isInteger(value) || value < 0)
      errors.push(
        `assessment.humanIntervention.${key} must be a non-negative integer`,
      );
  }
  requireStringArray(
    assessment.humanIntervention?.notes,
    "assessment.humanIntervention.notes",
    errors,
  );
  if (!Array.isArray(assessment.reportedCosts))
    errors.push("assessment.reportedCosts must be an array");
  else {
    assessment.reportedCosts.forEach((item, index) => {
      requireString(
        item?.kind,
        `assessment.reportedCosts[${index}].kind`,
        errors,
      );
      if (
        ...
        errors.push(
          `assessment.reportedCosts[${index}].amount must be a non-negative number`,
        );
      }
      requireString(
        item?.currency,
        `assessment.reportedCosts[${index}].currency`,
        errors,
      );
      requireString(
        item?.status,
        `assessment.reportedCosts[${index}].status`,
        errors,
      );
      requireString(
        item?.source,
        `assessment.reportedCosts[${index}].source`,
        errors,
      );
    });
  }
  if (!Array.isArray(assessment.references))
    errors.push("assessment.references must be an array");
  else {
    assessment.references.forEach((item, index) => {
      requireString(item?.kind, `assessment.references[${index}].kind`, errors);
      requireString(
        item?.value,
        `assessment.references[${index}].value`,
        errors,
      );
    });
  }
  for (const key of ["approach"])
    requireString(
      assessment.narrative?.[key],
      `assessment.narrative.${key}`,
      errors,
    );
  for (const key of [
  ...
  if (
    ...
    assessment.recommendations.length > 5
  ) {
    ...
    assessment.recommendations.forEach((item, index) => {
      for (const field of [
        ...
        requireString(
          item?.[field],
          `assessment.recommendations[${index}].${field}`,
          errors,
        );
      }
    });
  }
  if (!confidenceLevels.has(assessment.confidence?.level))
    errors.push("assessment.confidence.level is invalid");
  requireStringArray(
  ...
  if (/replace with/i.test(serialized))
    errors.push("assessment still contains template placeholders");
  if (
...
function scopeSummary(scope) {
  return Object.fromEntries(
    [...scopeStatuses].map((status) => [
      status,
      scope.filter((item) => item.status === status).length,
    ]),
  );
}
...
function validateExternalTimingAttribution(assessment, telemetry) {
  ...
  const operationalIntervals = Array.isArray(
  ...
  assessment.externalTiming.forEach((entry, index) => {
    ...
    if (
      ...
      errors.push(
        `assessment.externalTiming[${index}] falls outside the measurement boundary`,
      );
    }
    if (
      operationalIntervals.some((active) => intervalsOverlap(interval, active))
    ) {
      errors.push(
        `assessment.externalTiming[${index}] overlaps agent operational time`,
      );
    }
  });
  return errors;
...
export function renderMarkdown(measurement) {
  ...
  const tokenTotal =
    telemetry.tokens.allAgentsTotal ?? telemetry.tokens.total ?? "unavailable";
  const lines = [
    `# ${assessment.title}`,
    "",
    ...
    markdownList(
      assessment.errors,
      (entry) =>
        `**${entry.category}** — ${entry.summary}. Impact: ${entry.impact}. Corrected: ${entry.corrected ? "yes" : "no"}.`,
    ),
    "",
    "## External timing",
    "",
    markdownList(
      ...
      (entry) =>
        `**${entry.kind}** — ${entry.label}: ${formatDuration(entry.durationMs)} (${entry.status}; ${entry.overlap}; ${entry.verified ? "verified" : "unverified"}).`,
    ),
    "",
    "## Retries",
    "",
    markdownList(
      assessment.retries,
      (entry) =>
        `**${entry.kind}** — ${entry.count} retries. Reason: ${entry.reason}.`,
    ),
    "",
    "## What worked",
    "",
    ...
    markdownList(
      ...
      (item) =>
        `${item.kind}: ${item.amount} ${item.currency} (${item.status}) — ${item.source}`,
    ),
    "",
    "## References",
    "",
...
async function writePrivateImmutable(path, contents) {
  ...
  try {
    await stat(path);
    throw new Error(
      `Refusing to overwrite existing measurement artifact: ${path}`,
    );
  } catch (error) {
    if (!(error instanceof Error && "code" in error && error.code === "ENOENT"))
      throw error;
...
export async function persistMeasurement(options) {
  ...
  if (options.telemetry?.schemaVersion !== 2) {
    ...
    errors.push(
      ...validateExternalTimingAttribution(
        options.assessment,
        options.telemetry,
      ),
    );
  ...
  const externalWaitOutsideOperationalMs = unionDurationMs(
    options.assessment.externalTiming
      .filter(
        (entry) => entry.overlap === "outside-active-turn" && entry.verified,
      )
  ...
  const derived = {
    ...
    retries: options.assessment.retries.reduce(
  ...
  const repoHash = sha256(
    options.telemetry.repository.root ?? options.telemetry.repository.name,
  ).slice(0, 10);
...
export function validateMeasurement(measurement) {
  ...
  if (!uuidPattern.test(measurement.measurementId ?? ""))
    errors.push("measurement.measurementId is invalid");
  if (
    !isRecord(measurement.telemetry) ||
    measurement.telemetry.schemaVersion !== (legacy ? 1 : 2)
  ) {
  ...
  if (!legacy) {
    for (const field of [
      "operationalMs",
      "threadSpanMs",
      "unattributedBetweenTurnMs",
    ]) {
      const value = measurement.telemetry?.timing?.[field];
      if (typeof value !== "number" || !Number.isFinite(value) || value < 0) {
        errors.push(
          `measurement.telemetry.timing.${field} must be a non-negative number`,
        );
      }
    }
    for (const field of [
      ...
      if (typeof value !== "number" || !Number.isFinite(value) || value < 0) {
        errors.push(
          `measurement.derived.${field} must be a non-negative number`,
        );
      }
    }
  }
  errors.push(
    ...validateAssessment(
      legacy
        ? normalizeLegacyAssessment(measurement.assessment)
        : measurement.assessment,
    ),
  );
  if (
    !isRecord(measurement.integrity) ||
    measurement.integrity.algorithm !== "sha256"
  ) {
    ...
    if (integrity.contentSha256 !== actual)
      errors.push("measurement integrity hash does not match content");
  }
  return errors;
}
...
async function resolveSessionOptions(options) {
  const sessionsRoot = resolve(
    optionString(options, "sessions-root") ??
      resolve(homedir(), ".codex", "sessions"),
  );
  const threadId =
    optionString(options, "thread-id") ?? process.env.CODEX_THREAD_ID;
  ...
  const sessionPath = explicitSession
    ...
    : threadId
      ...
      : null;
  if (!sessionPath)
    throw new Error("Set CODEX_THREAD_ID or pass --thread-id/--session");
...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (command === "collect") {
    ...
    return output
      ? { ok: true, operation: "collect", output: resolve(output), telemetry }
      : telemetry;
  }
  if (command === "finalize") {
    const assessmentPath = optionString(options, "assessment");
    if (!assessmentPath)
      throw new Error("finalize requires --assessment <path>");
    const sessionOptions = await resolveSessionOptions(options);
    const telemetry = await collectTelemetry({
      ...sessionOptions,
      cutoff: optionString(options, "cutoff") ?? "latest-user",
    ...
    const assessment = JSON.parse(
      await readFile(resolve(assessmentPath), "utf8"),
    );
    return persistMeasurement({
      assessment,
      telemetry,
      outputRoot: resolve(
        optionString(options, "output-root") ??
          resolve(homedir(), ".development-system", "measurements"),
      ),
    });
  }
  if (command === "validate") {
