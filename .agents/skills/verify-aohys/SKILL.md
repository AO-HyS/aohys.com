...
The starting model remains the orchestrator and selects the executor and
verification judge from available models within the user's provider limits.
Roster entries recommend defaults; they do not prove runtime capability.
Execution and judgment remain separate roles. The runner needs actual Computer
Use capability, and the judge needs vision for screenshot-based acceptance.
Report BLOCKED when a required capability is unavailable; do not silently
switch provider families.

The selected orchestrator creates a versioned `execution-plan.json` before dispatch. Plans are
...
bound only by `rubricSha256`. The neutral Computer Use
...
before/after probes, private evidence, a verification verdict, and cleanup all exist
...
The verification judge owns PASS, FAIL, BLOCKED, and INCONCLUSIVE; files, static checks, and a
