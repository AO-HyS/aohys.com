...
export const WORKING_BACKWARDS_PHASES = Object.freeze([
  {
    role: "product-grill",
    label: "Product Grill With Docs",
    checkpoint: "document",
    fileStem: "product-grill",
  },
  {
    role: "customer-story",
    label: "Future Customer Story",
    checkpoint: "document",
    fileStem: "customer-story",
  },
  {
    role: "technical-grill",
    label: "Technical Grill With Docs",
    checkpoint: "document",
    fileStem: "technical-grill",
  },
  {
    role: "research-questions",
    label: "Research Questions",
    checkpoint: "document",
    fileStem: "research-questions",
  },
  {
  ...
  },
  {
    ...
    checkpoint: "product",
    fileStem: "product-contract",
  },
  {
    role: "technical-contract",
    label: "Technical Contract",
    checkpoint: "technical",
    fileStem: "technical-contract",
  },
  {
    role: "implementation-map",
    label: "Implementation Map",
    checkpoint: "implementation",
    fileStem: "implementation-map",
  },
  {
    role: "t3-handoff",
    label: "T3 Handoff",
...
export const ARCHITECTURE_CONVERGENCE_DIMENSIONS = Object.freeze([
  "repository-map",
  "module-boundaries",
...
const negativePattern =
  /\b(?:no|nunca|todavia no|aun no|rechazo|revoco|denegado|not|never|reject|revoke|don't|do not)\b/iu;
const ambiguousPattern =
  /\b(?:quiza|tal vez|a lo mejor|supongo|maybe|perhaps|probably)\b/iu;
const historicalPattern =
  /\b(?:habia|anteriormente|previamente|previously|already)\b|\bya\b[\s\S]*\b(?:aprobad|approved)\b/iu;
const revisionPattern =
  /\b(?:pero|aunque|excepto|cambia|cambiar|ajusta|ajustar|corrige|corregir|modifica|modificar|however|but|except|change|adjust|revise|fix)\b/iu;
const noChangeApprovalPattern =
  /\b(?:no\s+(?:tengo|hay|necesito|quiero|veo)\s+(?:ningun(?:a|os|as)?\s+)?(?:cambios?|ajustes?|correcciones?|comentarios?)|no\s+hace\s+falta\s+(?:cambiar|ajustar|corregir)\s+nada|sin\s+cambios?|nada\s+que\s+(?:cambiar|ajustar|corregir))\b/iu;
const firstPersonApprovalPattern =
  /^(?:yo\s+)?(?:apruebo|lo apruebo|i approve)(?=\b|[,.!])/iu;
const standaloneApprovalPattern =
  /^(?:aprobado|approved)(?:[,.]?\s*(?:sigue|continua|adelante|avanza|next|continue|go ahead))?[.!]?$/iu;
const directConversationalApprovalPattern =
  /^(?:si|yes|claro|perfecto(?:\s+todo(?:\s+aqui)?)?|excelente|listo|correcto|muy bien|todo(?:\s+(?:esto|lo\s+recomendado))?\s+(?:esta\s+)?(?:muy\s+)?bien|todo correcto|de acuerdo|se ve (?:muy )?bien|esta (?:muy )?bien|me (?:gusta(?:\s+como\s+quedo)?|parece (?:muy )?(?:bien|correcto|perfecto))|por mi (?:esta )?(?:muy )?(?:bien|correcto|perfecto)|lo veo (?:muy )?bien|ya quedo|suena (?:muy )?bien|looks good|all good|okay|ok)[.!]*$/iu;
const conversationalApprovalPattern =
  /\b(?:si|yes|claro|perfecto|listo|correcto|todo correcto|todo bien|de acuerdo|se ve bien|esta bien|me funciona|me parece (?:muy )?(?:bien|correcto|perfecto)|por mi (?:esta )?(?:muy )?(?:bien|correcto|perfecto)|lo veo (?:muy )?bien|ya quedo|looks good|all good|okay|ok)\b/iu;
const advancePattern =
  /\b(?:sigue|seguir|continua|continuar|adelante|avanza|avanzar|dale|date|vamos a (?:lo que sigue|la siguiente (?:fase|etapa))|pasemos al siguiente|puedes (?:seguir|continuar|avanzar|pasar al siguiente)|next|continue|go ahead|move on)\b/iu;
const standaloneAdvancePattern =
  /^(?:adelante|dale|date|continua(?:\s+con\s+(?:el|la)\s+siguiente\s+(?:documento|fase|etapa|paso))?|sigue(?:\s+con\s+lo\s+que\s+sigue)?|vamos\s+a\s+(?:lo\s+que\s+sigue|la\s+siguiente\s+(?:fase|etapa))|pasemos\s+al\s+siguiente|next|continue|go ahead|move on)[.!]*$/iu;
const formalGateTermPattern =
  /\b(?:producto|product|tecnico|technical|tickets?|implementation map|mapa de implementacion)\b/giu;
const quotedOrReportedPattern =
  /["“”'‘’]|\b(?:dijo|dice|dicen|menciono|me dijeron|said|says|reported|reports)\b/iu;
const managedReaderMarker =
...
export function classifyApproval(message) {
  ...
  const semantic = normalized
    ...
    .toLowerCase();
  const safetySemantic = semantic
    .replace(noChangeApprovalPattern, " ")
    .replace(/\s+/g, " ")
    .trim();
  const formalGateTerms = new Set(
    [...semantic.matchAll(formalGateTermPattern)].map((match) => match[0]),
  );
  const accepted =
    normalized.length > 0 &&
    ...
    (firstPersonApprovalPattern.test(semantic) ||
      ...
      (noChangeApprovalPattern.test(semantic) &&
        advancePattern.test(semantic)) ||
      (conversationalApprovalPattern.test(semantic) &&
        advancePattern.test(semantic)));
  return {
    accepted,
    normalized,
    reason: accepted
      ? "explicit-approval-at-active-checkpoint"
      : "feedback-or-ambiguous-language",
  };
}
...
function workflowSlug(workspaceDir) {
  ...
  if (!normalized)
    throw new Error(
      "T3 Code Working Backwards workspace must have a path-safe slug",
    );
  return normalized;
}
...
function isManagedTechnicalReader(html) {
  return (
    html.includes(managedReaderMarker) ||
    legacyManagedReaderSignature.every((part) => html.includes(part))
  );
}
...
async function inspectTechnicalReaderOwnership(path) {
  try {
    ...
    if (entry.isSymbolicLink())
      return { managed: false, collision: { path, reason: "symbolic-link" } };
    if (!entry.isFile())
      return {
        managed: false,
        collision: { path, reason: "non-regular-entry" },
      };
    const html = await readFile(path, "utf8");
    if (!isManagedTechnicalReader(html))
      return {
        managed: false,
        collision: { path, reason: "unmanaged-regular-file" },
      };
...
function technicalReaderDestinationCollision(collision) {
  return Object.assign(
    new Error(
      `Technical Reader destination collision (${collision.reason}): ${collision.path}`,
    ),
    {
...
async function retireLegacyWorkflowReader(status) {
  ...
  if (
    legacyReaderPath === status.readerPath ||
    legacyReaderPath === status.readerLibraryPath
  ) {
  ...
  if (ownership.collision)
    return {
      retiredReaderPath: null,
      readerIndexCollision: ownership.collision,
    };
...
function localRelativeHref(fromDir, target) {
  return relative(fromDir, target)
    .split(sep)
...
function roleFromFrontmatter(content) {
  ...
  const role =
    match[1].match(
      /^working_backwards_role:\s*['"]?([a-z0-9-]+)['"]?\s*$/imu,
    )?.[1] ?? null;
  return WORKING_BACKWARDS_PHASES.some((phase) => phase.role === role)
    ? role
    : null;
}
...
function assertPrivateWorkspacePath(home, workspaceDir) {
  ...
  if (
    ...
    throw new Error(
      "T3 Code Working Backwards workspace must be inside private Development System HOME",
    );
  }
...
async function readArtifacts(workspaceDir) {
  ...
  for (const entry of entries
    ...
    if (role === "t3-handoff")
      throw new Error(
        "The T3 handoff is private and cannot live in the T3 Code planning workspace",
      );
    if (byRole[role])
      throw new Error(
        `T3 Code Working Backwards workspace has more than one ${role} artifact`,
      );
    byRole[role] = {
      role,
      path,
  ...
  return WORKING_BACKWARDS_PHASES.flatMap((phase) =>
    byRole[phase.role] ? [byRole[phase.role]] : [],
  );
}
...
function validGateReceipt(receipt) {
  if (
    ...
    !["product", "technical", "implementationMap"].includes(
      String(receipt.gate),
    )
  )
    return false;
  if (
    ...
    normalizeRepositoryIdentity(receipt.repositoryIdentity) !==
...
const lifecycleTransitions = Object.freeze({
  product: [
    [
      "start_requirements",
      "requirements_in_progress",
      "Inicia Working Backwards desde la historia futura del usuario",
    ],
    [
      "approve_requirements",
      "requirements_approved",
      "Apruebo el Product Contract de Working Backwards",
    ],
  ],
  technical: [
    [
      "create_spec_plan",
      "spec_plan_ready",
      "Genera el Technical Contract de Working Backwards",
    ],
    [
      "approve_spec_plan",
      "spec_plan_approved",
      "Apruebo el Technical Contract de Working Backwards",
    ],
  ],
  implementationMap: [
    [
      "create_tickets",
      "tickets_ready",
      "Genera el Implementation Map de Working Backwards",
    ],
    [
      "approve_tickets",
      "tickets_approved",
      "Apruebo el Implementation Map de Working Backwards",
    ],
  ],
});
...
async function withLifecycleLock(home, id, callback) {
  ...
  if (!(await existsWithoutSymlink(directory)))
  ...
  for (let attempt = 0; attempt < 50; attempt += 1) {
    try {
      ...
      if (attempt === 49)
        throw new Error("Lifecycle transition is already in progress");
...
async function executeLifecycleGate(home, id, gate, acceptedAt) {
  ...
  return withLifecycleLock(home, id, async () => {
    ...
    if (
      ...
      state.workflowId !== id ||
      !lifecycleStages.includes(state.stage) ||
    ...
    for (const [operation, stageAfter, request] of transitions) {
      if (
        lifecycleStages.indexOf(state.stage) >=
        lifecycleStages.indexOf(stageAfter)
      )
        continue;
...
async function invalidateLifecycleAfterDrift(home, id, invalidIndex, reason) {
  const productIndex = WORKING_BACKWARDS_PHASES.findIndex(
    (phase) => phase.role === "product-contract",
  );
  const technicalIndex = WORKING_BACKWARDS_PHASES.findIndex(
    (phase) => phase.role === "technical-contract",
  );
  const implementationIndex = WORKING_BACKWARDS_PHASES.findIndex(
    (phase) => phase.role === "implementation-map",
  );
  const allowedStage =
    invalidIndex <= productIndex
      ? "requirements_in_progress"
  ...
  await withLifecycleLock(home, id, async () => {
    ...
    if (
      ...
      existing.workflowId !== id ||
      !lifecycleStages.includes(existing.stage) ||
      !Array.isArray(existing.evidence) ||
      !Array.isArray(existing.authorizations)
    ) {
      throw new Error("Lifecycle state schema is invalid");
    }
    const stageMustRewind =
      lifecycleStages.indexOf(existing.stage) >
      lifecycleStages.indexOf(allowedStage);
    if (
      !stageMustRewind &&
      !existing.terminalSlice &&
      existing.authorizations.length === 0
    )
      return;
...
function implementationTickets(content) {
  ...
  const headings = [
    ...body.matchAll(
      /^##\s+`?([A-Za-z][A-Za-z0-9._-]*)`?(?:\s*(?:[-—:]|\s)\s*.*)?$/gmu,
    ),
  ];
...
function validateFirstExecutableSlice(content, firstSlice) {
  ...
  if (matches.length !== 1)
    throw new Error(
      `Implementation Map first slice must identify exactly one ticket heading: ${firstSlice}`,
    );
  const section = matches[0].body
    .normalize("NFD")
    .replace(/\p{Diacritic}/gu, "");
  const headings = [
    ...section.matchAll(
      /^#{3,4}[ \t]+(outcome|resultado|objetivo|acceptance|aceptacion|criterios? de aceptacion|checks?|verificacion|validacion|pruebas?|dependencies|dependencias)[ \t]*:?[ \t]*(.*)$/gimu,
    ),
  ];
  ...
  const aliases = new Map([
    ...
    ["criterio de aceptacion", "acceptance"],
    ["criterios de aceptacion", "acceptance"],
    ["check", "checks"],
    ["checks", "checks"],
    ["verificacion", "checks"],
    ["validacion", "checks"],
  ...
  const placeholder =
    /^(?:[-*]\s*)?(?:todo|tbd|pendiente|por definir|n\/a)?[.!]?$/iu;
  const missing = ["outcome", "acceptance", "checks", "dependencies"].filter(
    (key) => !bodies[key] || placeholder.test(bodies[key]),
  );
  if (missing.length > 0)
    throw new Error(
      `Implementation Map first slice is not executable; missing: ${missing.join(", ")}`,
    );
  const dependencies = bodies.dependencies.replace(/^[-*]\s*/gmu, "").trim();
  if (
    !/^(?:none|ninguna?s?|no aplica|n\/a|\[\])(?:[.!])?$/iu.test(dependencies)
  ) {
    throw new Error(
      "Implementation Map first slice must have no unresolved dependencies",
    );
  }
  return matches[0];
}
...
/** @param {string} content */
function validateArchitectureConvergenceTechnicalGrill(content) {
  if (
    frontmatterValue(content, "working_backwards_program") !==
  ...
  for (const line of content.split(/\r?\n/u)) {
    ...
    if (
      cells.length < 4 ||
      !ARCHITECTURE_CONVERGENCE_DIMENSIONS.includes(cells[0])
    )
      continue;
    if (rows.has(cells[0])) duplicates.add(cells[0]);
    rows.set(cells[0], cells.slice(1, 4));
  }
  const placeholder =
    /^(?:|[-—]|todo|tbd|pendiente|por definir|unknown|desconocido)$/iu;
  const disposition =
    /\b(?:keep|change|remove|unproven|conservar|cambiar|retirar|eliminar|no demostrado)\b/iu;
  const missing = ARCHITECTURE_CONVERGENCE_DIMENSIONS.filter((dimension) => {
    const cells = rows.get(dimension);
    return (
      !cells ||
      cells.some((cell) => placeholder.test(cell)) ||
      !disposition.test(cells[1])
    );
  });
  if (missing.length > 0 || duplicates.size > 0) {
    const details = [
      missing.length > 0 ? `missing dimensions: ${missing.join(", ")}` : null,
      duplicates.size > 0
        ? `duplicate dimensions: ${[...duplicates].join(", ")}`
        : null,
    ]
      .filter(Boolean)
      .join("; ");
    throw new Error(
      `Architecture convergence Technical Grill is incomplete; ${details}`,
    );
  }
}
...
function validPrivateHandoff(content, expected) {
  const receiptByGate = new Map(
    expected.receipts.map((receipt) => [receipt.gate, receipt]),
  );
  ...
  if (
    Object.entries(required).some(
      ([key, value]) => frontmatterValue(content, key) !== value,
    )
  )
    return false;
  const firstSlice = frontmatterValue(content, "first_slice");
  const approvedFirstSlice = expected.implementationMapContent
    ? frontmatterValue(
        expected.implementationMapContent,
        "working_backwards_first_slice",
      )
    : null;
  if (
    !firstSlice ||
    !approvedFirstSlice ||
    firstSlice !== approvedFirstSlice ||
    !expected.implementationMapContent
  )
    return false;
  try {
    validateFirstExecutableSlice(
      expected.implementationMapContent,
      approvedFirstSlice,
    );
    return true;
...
export async function inspectT3Workflow(input) {
  ...
  const declaredInitiative =
    artifacts
      .map(
        (artifact) =>
          frontmatterValue(artifact.content, "initiative_name") ??
          frontmatterValue(artifact.content, "initiative"),
      )
      .find(Boolean) ?? null;
  const initiativeName =
    input.initiativeName?.trim() ||
    (typeof persisted?.initiativeName === "string"
      ? persisted.initiativeName.trim()
      : "") ||
    declaredInitiative ||
    humanizeWorkflowSlug(slug);
  ...
  const savedGateReceipts = approvals(gateReceiptFile?.receipts).filter(
    validGateReceipt,
  );
  ...
  try {
    ...
    const valid = validPrivateHandoff(content, {
      ...
      implementationMapContent:
        artifactByRole.get("implementation-map")?.content ?? null,
    });
    if (valid)
      artifactByRole.set("t3-handoff", {
        role: "t3-handoff",
        path: privateHandoffPath,
        fileName: "t3-handoff.md",
        content,
        contentHash: hashValue(content),
      });
  ...
  for (let index = 0; index < savedApprovals.length; index += 1) {
    ...
    if (!artifact || artifact.contentHash !== approval.contentHash) {
      invalidIndex = WORKING_BACKWARDS_PHASES.findIndex(
        (phase) => phase.role === approval.role,
      );
      break;
    }
  }
  ...
  if (observedIdentity && input.repositoryRevision) {
    for (const receipt of savedGateReceipts) {
      if (
        receipt.repositoryIdentity !== observedIdentity ||
        receipt.repositoryRevision !== input.repositoryRevision
      ) {
        const role =
          receipt.gate === "product"
            ? "product-contract"
            : receipt.gate === "technical"
              ? "technical-contract"
              : "implementation-map";
        const index = WORKING_BACKWARDS_PHASES.findIndex(
  ...
  if (invalidIndex >= 0)
    await invalidateLifecycleAfterDrift(
      home,
      id,
      invalidIndex,
      invalidReason ?? "working-backwards-drift",
    );
      ...
      phase?.checkpoint === "product"
        ...
        : phase?.checkpoint === "technical"
          ...
          : phase?.checkpoint === "implementation"
            ? savedGateReceipts.find(
                (receipt) => receipt.gate === "implementationMap",
              )
            : null;
    const formalReceiptMatches =
      !formalReceipt ||
      (Array.isArray(formalReceipt.artifacts) &&
        formalReceipt.artifacts.some(
          (snapshot) =>
            snapshot &&
            typeof snapshot === "object" &&
            snapshot.contentHash === approval.contentHash,
        ));
    const requiredFormalReceiptExists =
      !["product", "technical", "implementation"].includes(
        String(phase?.checkpoint),
      ) || Boolean(formalReceipt);
    return (
      index >= 0 &&
      (invalidIndex < 0 || index < invalidIndex) &&
      artifactByRole.get(approval.role)?.contentHash === approval.contentHash &&
      requiredFormalReceiptExists &&
      formalReceiptMatches
    );
  });
  const approvedRoles = new Set(
    effectiveApprovals.map((approval) => approval.role),
  );
  let currentPhase =
    WORKING_BACKWARDS_PHASES.find((phase) => {
  ...
  const action =
    currentPhase.role === "t3-handoff" && currentArtifact
      ? "handoff-ready"
  ...
  return {
    ...
    gateReceipts,
    invalidFrom:
      invalidIndex >= 0 ? WORKING_BACKWARDS_PHASES[invalidIndex].role : null,
    invalidReason,
...
function readerNextAction(action, phaseLabel) {
  ...
  if (action === "create-private-handoff")
    return "Crear el handoff privado para T3 Code";
  return "Working Backwards terminó; el handoff está listo para Implement Preview";
}

/**
 * @param {Awaited<ReturnType<typeof inspectT3Workflow>>} status
 * @param {{displayArtifact?: (typeof status.artifacts)[number] | null, readerDir?: string}} [options]
 */
export function renderT3Reader(status, options = {}) {
  const approved = new Set(
    status.documentApprovals.map((approval) => approval.role),
  );
  const artifactByRole = new Map(
    status.artifacts.map((artifact) => [artifact.role, artifact]),
  );
  const displayArtifact =
    options.displayArtifact !== undefined
      ? options.displayArtifact
      : status.currentPhase.role === "t3-handoff"
        ? null
        : (status.currentArtifact ?? status.artifacts.at(-1) ?? null);
  const displayPhase =
    WORKING_BACKWARDS_PHASES.find(
      (phase) => phase.role === displayArtifact?.role,
    ) ?? status.currentPhase;
  const displayIndex = Math.max(
    0,
    WORKING_BACKWARDS_PHASES.findIndex(
      (phase) => phase.role === displayPhase.role,
    ),
  );
  const phaseLabel = `Fase ${displayIndex + 1} de ${WORKING_BACKWARDS_PHASES.length}`;
  const readerDir = resolve(options.readerDir ?? status.workspaceDir);
  const displayIsActive =
    displayArtifact?.role === status.currentPhase.role &&
    status.action === "review-artifact";
...
Este documento todavía no se ha creado.`);
  const markdown = `> [!NOTE]\n> Este reporte muestra la fase activa de una iniciativa más amplia. Usa Historial para abrir cada documento anterior con el mismo Reader.\n\n${artifactMarkdown}`;
  const artifacts = WORKING_BACKWARDS_PHASES.map((phase) => {
    const artifact = artifactByRole.get(phase.role);
    const complete =
      approved.has(phase.role) ||
      (phase.role === "t3-handoff" && status.action === "handoff-ready");
    const active = phase.role === displayPhase.role;
    const historyPath = artifact
      ? resolve(
          status.workspaceDir,
          "reader-history",
          artifact.fileName.replace(/\.md$/iu, ".html"),
        )
      : null;
    return {
      id: phase.role,
      label: phase.label,
      state: active ? "active" : complete ? "complete" : "pending",
      fileName: artifact?.fileName ?? null,
      href: historyPath ? localRelativeHref(readerDir, historyPath) : null,
    };
  });
  const model = buildTechnicalReaderModel({
    ...
    workflow: {
      ...
      libraryHref: localRelativeHref(readerDir, status.readerLibraryPath),
      reportStatusLabel: phaseLabel,
    },
    document: {
      type: displayPhase.label,
      status: displayIsActive
        ? "En revisión"
        : displayArtifact
          ? "Aprobado"
          : status.action === "handoff-ready"
            ? "Listo"
            : "Pendiente",
      profile: status.profile,
      repository: status.repositoryIdentity,
      sourceFile: displayArtifact?.fileName ?? null,
      sourceHref: displayArtifact
        ? localRelativeHref(
            readerDir,
            resolve(status.workspaceDir, displayArtifact.fileName),
          )
        : null,
      markdown,
    },
...
async function writeReaderLibrary(input, status) {
  ...
  if (!isIsoDate(timestamp))
    throw new Error("Technical Reader library timestamp must be an ISO date");
  return withLifecycleLock(home, "technical-reader-library", async () => {
    for (const path of [
      status.readerLibraryCatalogPath,
      status.readerLibraryPath,
    ]) {
    ...
    if (
      existing &&
      (existing.schemaVersion !== 1 || !Array.isArray(existing.entries))
    ) {
      throw new Error("Technical Reader library catalog is invalid");
    }
    const priorEntries = Array.isArray(existing?.entries)
      ? existing.entries.filter((entry) => entry && typeof entry === "object")
      : [];
    const previous =
      priorEntries.find((entry) => entry.id === status.workflowId) ?? null;
    const declaredCreatedAt = status.artifacts
      .map(
        (artifact) =>
          frontmatterValue(artifact.content, "created_at") ??
          frontmatterValue(artifact.content, "createdAt"),
      )
      .find(Boolean);
    const entry = {
      id: status.workflowId,
      ...
      createdAt: readerLibraryDate(
        previous?.createdAt ?? declaredCreatedAt,
        timestamp,
      ),
      updatedAt: new Date(timestamp).toISOString(),
      nextAction: readerNextAction(status.action, status.currentPhase.label),
      readerHref: localRelativeHref(
        resolve(status.readerLibraryPath, ".."),
        status.readerPath,
      ),
    };
    const retained = priorEntries
      .filter((candidate) => candidate.id !== status.workflowId)
      .map((candidate) => ({
        ...
        phaseRole:
          typeof candidate.phaseRole === "string" ? candidate.phaseRole : "",
        ...
        nextAction:
          typeof candidate.nextAction === "string" ? candidate.nextAction : "",
        readerHref:
          typeof candidate.readerHref === "string" ? candidate.readerHref : "",
      }))
      .filter(
        (candidate) => candidate.id && candidate.slug && candidate.readerHref,
      );
    const catalog = {
      schemaVersion: 1,
    ...
    return {
      readerLibraryPath: status.readerLibraryPath,
      readerLibraryCatalogPath: status.readerLibraryCatalogPath,
    };
  });
}
...
export async function writeT3Reader(input) {
  ...
  for (const segment of [
    ...
    current = resolve(current, segment);
    if (!(await existsWithoutSymlink(current)))
      await mkdir(current, { mode: 0o700 });
    await chmod(current, 0o700);
  }
  const status = await inspectT3Workflow(input);
  const destinationOwnership = await inspectTechnicalReaderOwnership(
    status.readerPath,
  );
  if (destinationOwnership.collision)
    throw technicalReaderDestinationCollision(destinationOwnership.collision);
  const historyDir = resolve(status.workspaceDir, "reader-history");
  if (!(await existsWithoutSymlink(historyDir)))
    await mkdir(historyDir, { mode: 0o700 });
  await chmod(historyDir, 0o700);
  const historyEntries = status.artifacts.map((artifact) => ({
    artifact,
    path: resolve(historyDir, artifact.fileName.replace(/\.md$/iu, ".html")),
  }));
  for (const entry of historyEntries) {
    const ownership = await inspectTechnicalReaderOwnership(entry.path);
    if (ownership.collision)
      throw technicalReaderDestinationCollision(ownership.collision);
  }
  const temporary = `${status.readerPath}.${randomUUID()}.tmp`;
  await writeFile(temporary, renderT3Reader(status), {
    encoding: "utf8",
    mode: 0o600,
  });
  await chmod(temporary, 0o600);
  ...
  for (const entry of historyEntries) {
    const historyTemporary = `${entry.path}.${randomUUID()}.tmp`;
    await writeFile(
      historyTemporary,
      renderT3Reader(status, {
        displayArtifact: entry.artifact,
        readerDir: historyDir,
      }),
      { encoding: "utf8", mode: 0o600 },
    );
    await chmod(historyTemporary, 0o600);
    await rename(historyTemporary, entry.path);
  ...
  const readerHistoryPaths = historyEntries.map((entry) => entry.path);
  return {
    ...status,
    ...library,
    readerWritten: true,
    ...
    localWrites: [
      status.readerPath,
      ...readerHistoryPaths,
      library.readerLibraryCatalogPath,
      library.readerLibraryPath,
    ],
  };
}
...
async function existsWithoutSymlink(path) {
  try {
    ...
    if (stat.isSymbolicLink())
      throw new Error(
        `Unsafe symbolic link in Working Backwards private path: ${path}`,
      );
    return true;
  } catch (error) {
    if (error instanceof Error && "code" in error && error.code === "ENOENT")
...
export async function recordT3Turn(input) {
  ...
  if (
    !approval.accepted ||
    before.action !== "review-artifact" ||
    !before.currentArtifact ||
    before.currentPhase.checkpoint === null
  ) {
    return {
      ...before,
      approval: { ...approval, accepted: false, kind: null },
    };
  }
  const checkpoint = before.currentPhase.checkpoint;
  if (before.currentPhase.role === "technical-grill")
    validateArchitectureConvergenceTechnicalGrill(
      before.currentArtifact.content,
    );
  if (checkpoint === "implementation") {
    const firstSlice = frontmatterValue(
      before.currentArtifact.content,
      "working_backwards_first_slice",
    );
    if (!firstSlice)
      throw new Error(
        "Implementation Map approval requires working_backwards_first_slice in its frontmatter",
      );
    validateFirstExecutableSlice(before.currentArtifact.content, firstSlice);
  }
  const operation =
    checkpoint === "document" ? null : formalOperations[checkpoint];
  if (
    operation &&
    (!input.repositoryIdentity?.trim() || !input.repositoryRevision?.trim())
  ) {
    throw new Error(
      "formal Working Backwards approval requires repository identity and revision",
    );
  }
  const prior = await readJson(before.statePath);
  const phaseIndex = WORKING_BACKWARDS_PHASES.findIndex(
    (phase) => phase.role === before.currentPhase.role,
  );
  const retainedDocuments = before.documentApprovals.filter((entry) => {
    const index = WORKING_BACKWARDS_PHASES.findIndex(
      (phase) => phase.role === entry.role,
    );
    ...
    checkpoint === "product"
      ...
      : checkpoint === "technical"
        ...
        : checkpoint === "implementation"
          ? "implementationMap"
          : null;
  const repositoryIdentity = operation
    ? normalizeRepositoryIdentity(input.repositoryIdentity)
    : null;
  const currentArtifacts = before.artifacts.filter((artifact) => {
    const index = WORKING_BACKWARDS_PHASES.findIndex(
      (phase) => phase.role === artifact.role,
    );
    return index >= 0 && index <= phaseIndex;
  });
  const allSnapshots = operation
    ? canonicalArtifactSnapshots(
        currentArtifacts,
        before.workflowId,
        repositoryIdentity,
    ...
    : [];
  const governingSnapshots = gate
    ? allSnapshots.filter((artifact) => gateRoles[gate].includes(artifact.role))
    : [];
  ...
  const gateReceipt = gateBody
    ? {
        ...
        ...(gate === "implementationMap"
          ? {
              ticketMapHash:
                governingSnapshots.find(
                  (artifact) => artifact.role === "ticket-map",
                )?.contentHash ?? null,
            }
          : {}),
      }
  ...
  await writePrivateJson(
    ...
    before.statePath,
    {
      ...
      repositoryIdentity: operation
        ? repositoryIdentity
        : (prior?.repositoryIdentity ??
          (input.repositoryIdentity
            ? normalizeRepositoryIdentity(input.repositoryIdentity)
            : null)),
      repositoryRevision: operation
        ? input.repositoryRevision.trim()
        : (prior?.repositoryRevision ?? input.repositoryRevision ?? null),
      repositoryPath: input.repositoryPath
        ? resolve(input.repositoryPath)
        : (prior?.repositoryPath ?? null),
      approvedFirstSlice:
        checkpoint === "implementation"
          ? frontmatterValue(
              before.currentArtifact.content,
          ...
          : (prior?.approvedFirstSlice ?? null),
      documentApprovals: [...retainedDocuments, documentApproval],
      implementationAuthorized: false,
    },
  );
  if (gateReceipt) {
    await executeLifecycleGate(
      resolve(input.home ?? homedir()),
      before.workflowId,
      gate,
      acceptedAt,
...
function parseCli(argv) {
  ...
  for (let index = 0; index < tokens.length; index += 2) {
    ...
    else if (key === "--repository-identity")
      options.repositoryIdentity = value;
    else if (key === "--repository-revision")
      options.repositoryRevision = value;
    ...
    command === "status"
      ...
      : command === "turn"
        ...
        : command === "render"
          ...
          : (() => {
              throw new Error(
                "Usage: t3-workflow.mjs <status|turn|render> --workspace-dir <path> [options]",
              );
            })();
