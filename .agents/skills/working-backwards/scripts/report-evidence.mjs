...
function uri(asset, type) {
  const pattern =
    type === "image"
      ...
      : /^data:video\/(?:mp4|webm);base64,[A-Za-z0-9+/]+=*$/u;
  if (!pattern.test(asset?.dataUrl ?? ""))
    throw new Error("Report evidence requires normalized embedded media");
