...
export function visualImage(svg) {
  if (
    ...
    throw new Error("Visual requires a complete SVG image smaller than 2 MB");
  if (
    /<(?:script|foreignObject|iframe|object|embed)\b|<!DOCTYPE|<!ENTITY|\son[a-z]+\s*=/iu.test(
      svg,
    )
  )
    throw new Error("Visual contains active content");
  if (
    /\b(?:href|src)\s*=\s*["'](?!#)/iu.test(svg) ||
    [...svg.matchAll(/url\(([^)]*)\)/giu)].some(
      (match) => !/^["']?#[\w:.-]+["']?$/u.test(match[1].trim()),
    ) ||
    /@import/iu.test(svg)
  )
    throw new Error("Visual must not load external resources");
  const box =
    /viewBox\s*=\s*["']\s*(-?[\d.]+)\s+(-?[\d.]+)\s+([\d.]+)\s+([\d.]+)\s*["']/u.exec(
      svg,
    );
  if (
    !box ||
    !Number.isFinite(Number(box[3])) ||
    !Number.isFinite(Number(box[4])) ||
    Number(box[3]) <= 0 ||
    Number(box[4]) <= 0
  )
    throw new Error("Visual requires a finite, positive viewBox");
  // PR Lens supplies geometry; the shared presentation supplies reading size.
  const readable = svg.replace(
    "</svg>",
    "<style>.ntitle{font-size:17px;fill:#eef1f5}.nsub{font-size:13px;font-family:system-ui,sans-serif;fill:#bfc6d0}.lanelabel{font-size:10px;fill:#c4cbd5}.ltext{fill:#d5dbe3}</style></svg>",
  );
  const still = readable
    .replace(/<circle\b[^>]*>\s*<animateMotion\b[\s\S]*?<\/circle>/gu, "")
    .replace(/<animate(?:Motion|Transform)?\b[^>]*\/>/gu, "");
  const uri = (/** @type {string} */ value) =>
    `data:image/svg+xml;base64,${Buffer.from(value).toString("base64")}`;
  return {
    animated: uri(readable),
    still: uri(still),
    width: Number(box[3]),
    height: Number(box[4]),
    hasMotion: readable !== still,
  };
}
...
function graph(block, model, language) {
  let reference;
  try {
    reference = JSON.parse(block.source);
  } catch {
    throw new Error("pr-lens fence must contain an object with an id");
  }
  const visual = (model.visuals ?? []).find(
    (/** @type {Record<string, any>} */ item) => item.id === reference?.id,
  );
  if (!visual)
    throw new Error(`Missing PR Lens visual: ${string(reference?.id)}`);
...
function bars(block, language) {
  ...
  if (
    ...
    throw new Error(
      "Chart labels and finite numeric values must have equal lengths (1–40)",
    );
  const min = Math.min(0, ...data.values),
    max = Math.max(0, ...data.values),
    span = max - min || 1;
  const zero = (-min / span) * 100;
  const precision = Number.isInteger(data.precision)
    ? Math.min(6, Math.max(0, data.precision ?? 1))
    : 1;
  const number = new Intl.NumberFormat(language === "es" ? "es-MX" : "en-US", {
    maximumFractionDigits: precision,
  });
  const title =
    string(data.title) || (language === "es" ? "Comparación" : "Comparison");
  return `<figure class="document-chart"><figcaption><strong>${esc(title)}</strong>${data.unit ? `<span>${esc(data.unit)}</span>` : ""}</figcaption><ol class="bar-list">${data.labels
    .map((label, index) => {
      const value = data.values[index],
        left = value < 0 ? zero + (value / span) * 100 : zero;
      return `<li><div class="bar-heading"><span>${esc(label)}</span><strong>${number.format(value)}${data.unit ? ` <small>${esc(data.unit)}</small>` : ""}</strong></div><div class="bar-track" aria-hidden="true"><span class="bar-fill${index === 0 ? " is-primary" : ""}" style="left:${left}%;width:${(Math.abs(value) / span) * 100}%"></span>${min < 0 ? `<i class="bar-zero" style="left:${zero}%"></i>` : ""}</div>${Array.isArray(data.annotations) && data.annotations[index] ? `<p class="bar-note">${esc(data.annotations[index])}</p>` : ""}</li>`;
    })
    .join(
      "",
