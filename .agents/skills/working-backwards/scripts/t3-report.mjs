...
export async function writeTechnicalReport(options) {
  ...
  const html = renderTechnicalReaderHtml(
    buildTechnicalReaderModel({ ...input, presentation: "report" }),
  );
  let existingStat = null;
  try {
    existingStat = await lstat(outputPath);
    if (!existingStat.isFile() || existingStat.nlink !== 1)
      throw new Error("Report output must be a regular file with no aliases");
    const existing = await readFile(outputPath, "utf8");
    if (
      !existing.includes(
        '<meta name="generator" content="development-system-technical-reader">',
      ) ||
      !existing.includes('<body class="reader-report">')
    ) {
      throw new Error(
        "Refusing to overwrite a file that is not a managed report",
      );
    }
  } catch (error) {
  ...
  if (!existingStat) {
    ...
    try {
      ...
      if (
        !current.isFile() ||
        current.nlink !== 1 ||
        current.ino !== existingStat.ino ||
        current.mtimeMs !== existingStat.mtimeMs
      ) {
        throw new Error(
          "Report changed during generation; preserve it and retry",
        );
      }
...
function parseReportArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    if (
      !["--input", "--markdown", "--output"].includes(key) ||
      !value ||
      value.startsWith("--") ||
      parsed[key]
    ) {
      throw new Error(
        "Use --input metadata.json [--markdown report.md] --output report.html",
      );
    }
    parsed[key] = value;
  }
...
if (invokedAsCli) {
  ...
  if (
    ...
    (cliOptions["--markdown"] &&
      outputPath === resolve(cliOptions["--markdown"]))
  ) {
    throw new Error("Output must be a separate .html file");
  }
  const input = JSON.parse(await readFile(inputPath, "utf8"));
  if (cliOptions["--markdown"])
    input.document = {
      ...input.document,
      markdown: await readFile(resolve(cliOptions["--markdown"]), "utf8"),
    };
  process.stdout.write(
    JSON.stringify(await writeTechnicalReport({ input, outputPath })) + "\n",
  );
}
