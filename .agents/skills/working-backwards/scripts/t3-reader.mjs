...
function renderTable(block) {
  ...
  return `<div class="table-scroll" tabindex="0"><table class="data-table"><thead><tr>${columns.map((column, index) => `<th scope="col" style="text-align:${alignments[index] ?? "left"}">${inlineMarkdown(String(column))}</th>`).join("")}</tr></thead><tbody>${rows.map((row) => `<tr>${columns.map((column, index) => `<td data-label="${escapeHtml(plainInline(String(column)))}" style="text-align:${alignments[index] ?? "left"}">${inlineMarkdown(String(row[index] ?? ""))}</td>`).join("")}</tr>`).join("")}</tbody></table></div>`;
}
...
let fullscreenFigure=null;
const minimumReadableScale=.875;
...
const centerDiagramScroll=viewport=>{viewport.scrollLeft=Math.max(0,(viewport.scrollWidth-viewport.clientWidth)/2);viewport.scrollTop=Math.max(0,(viewport.scrollHeight-viewport.clientHeight)/2)};
const resetDiagram=(figure,animate=true)=>{const instance=currentPanzoom(figure);const viewport=figure.querySelector(".diagram-viewport");if(!instance||!viewport)return;instance.reset({animate});figure.dataset.zoom="1";figure.dataset.fitScale="1";requestAnimationFrame(()=>centerDiagramScroll(viewport))};
const fitDiagram=(figure,animate=true)=>{const instance=currentPanzoom(figure);const viewport=figure.querySelector(".diagram-viewport");const svg=figure.querySelector(".diagram-canvas svg");if(!instance||!viewport||!svg)return;instance.reset({animate:false});const viewBox=svg.viewBox&&svg.viewBox.baseVal;const naturalWidth=viewBox&&viewBox.width?viewBox.width:Math.max(1,svg.getBoundingClientRect().width);const naturalHeight=viewBox&&viewBox.height?viewBox.height:Math.max(1,svg.getBoundingClientRect().height);const horizontal=Math.max(1,viewport.clientWidth-48);const vertical=Math.max(1,viewport.clientHeight-48);const containScale=Math.min(1,horizontal/naturalWidth,vertical/naturalHeight);const scale=Math.max(minimumReadableScale,containScale);instance.zoom(scale,{animate});instance.pan(0,0,{animate,force:true});figure.dataset.zoom=String(scale);figure.dataset.fitScale=String(scale);requestAnimationFrame(()=>centerDiagramScroll(viewport))};
const zoomDiagram=(figure,direction)=>{const instance=currentPanzoom(figure);if(!instance)return;const scale=instance.getScale();const next=Math.max(.5,Math.min(4,scale+(direction*.2)));instance.zoom(next,{animate:true});figure.dataset.zoom=String(next)};
const setupPanzoom=figure=>{const viewport=figure.querySelector(".diagram-viewport");const svg=figure.querySelector(".diagram-canvas svg");if(!viewport||!svg||typeof Panzoom!=="function")return;const prior=currentPanzoom(figure);if(prior)prior.destroy();const viewBox=svg.viewBox&&svg.viewBox.baseVal;const naturalWidth=Math.max(280,viewBox&&viewBox.width?viewBox.width:960);svg.removeAttribute("height");svg.style.maxWidth="none";svg.style.width=naturalWidth+"px";svg.style.height="auto";svg.dataset.naturalWidth=String(naturalWidth);svg.setAttribute("role","img");svg.setAttribute("aria-label",figure.querySelector(".visual-title")?.textContent?.trim()||"Mermaid diagram");const instance=Panzoom(svg,{maxScale:4,minScale:.5,step:.2,canvas:true,pinchAndPan:true,animate:true,duration:180,excludeClass:"diagram-control"});panzoomByDiagram.set(figure,instance);figure.dataset.zoom="1";viewport.onwheel=event=>{event.preventDefault();if(event.ctrlKey||event.metaKey){instance.zoomWithWheel(event);figure.dataset.zoom=String(instance.getScale());return}const pan=instance.getPan();instance.pan(pan.x-event.deltaX,pan.y-event.deltaY,{force:true})};requestAnimationFrame(()=>fitDiagram(figure,false))};
...
document.addEventListener("fullscreenchange",()=>{const active=document.fullscreenElement&&document.fullscreenElement.matches("[data-mermaid]")?document.fullscreenElement:null;const figure=active||fullscreenFigure;document.querySelectorAll('[data-action="fullscreen"]').forEach(button=>button.setAttribute("aria-pressed",String(Boolean(active&&button.closest("[data-mermaid]")===active))));if(figure)requestAnimationFrame(()=>fitDiagram(figure));fullscreenFigure=active});
...
.reader-topbar{position:sticky;z-index:40;top:0;display:flex;align-items:center;justify-content:space-between;gap:1rem;min-width:0;min-height:4rem;padding:.65rem clamp(1rem,2.2vw,2rem);overflow:hidden;border-bottom:1px solid color-mix(in srgb,var(--line) 72%,transparent);background:var(--page)}.reader-identity,.reader-actions{display:flex;align-items:center;gap:.75rem;min-width:0}.reader-mark{display:grid;flex:none;place-items:center;width:2rem;height:2rem;border:1px solid var(--accent);border-radius:50%;color:var(--accent);font-size:.75rem;font-weight:750;letter-spacing:.02em}.reader-identity{flex:1;font-size:.95rem;overflow:hidden}.reader-identity strong{max-width:min(34vw,28rem)}.reader-identity strong,.reader-identity span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.reader-identity>span:last-child{color:var(--muted);font-size:.875rem}.reader-actions{flex:none;justify-content:flex-end}.report-state,.authority-state{display:inline-flex;align-items:center;gap:.5rem;color:var(--muted);font-size:.875rem}.report-state{color:var(--ink);font-weight:600}.report-state::before,.authority-state::before{content:"";flex:none;width:.5rem;height:.5rem;border-radius:50%;background:var(--accent)}.authority-state::before{background:var(--warn)}.authority-state.is-authorized::before{background:var(--ok)}.source-button{display:inline-flex;align-items:center;min-height:2.75rem;padding:.35rem .75rem;border-radius:.35rem;color:var(--muted);font-size:.875rem;text-decoration:none}.source-button:hover{color:var(--ink);background:var(--accent-soft)}.icon-button{display:grid;flex:none;place-items:center;min-width:2.75rem;min-height:2.75rem;border:0;border-radius:.35rem;background:transparent;cursor:pointer}.icon-button:hover{background:var(--accent-soft)}
.reader-layout{display:grid;grid-template-columns:minmax(10.5rem,14rem) minmax(0,56rem) minmax(10rem,13rem);align-items:start;gap:clamp(1rem,2vw,2.25rem);max-width:91rem;margin:0 auto;padding:0 clamp(.75rem,1.5vw,1.5rem)}.document-column{position:relative;grid-column:2;min-width:0;padding:clamp(3rem,6vw,5.25rem) clamp(1rem,3vw,2.5rem) 7rem}.document-column::before{content:"";position:absolute;inset:1.75rem 0 3rem;border-radius:.9rem;background:var(--document);box-shadow:0 0 0 1px color-mix(in srgb,var(--line) 42%,transparent);pointer-events:none}.artifacts-rail{grid-column:1;grid-row:1}.toc-rail{grid-column:3;grid-row:1}.artifacts-rail,.toc-rail{position:sticky;top:4rem;max-height:calc(100vh - 4rem);overflow:auto;padding:2.5rem 0;color:var(--muted)}.artifacts-rail{padding-left:.2rem}.toc-rail{padding-right:.2rem}
.document-header,.document-body{position:relative;min-width:0}
...
.diagram-viewport{position:relative;display:grid;place-items:stretch;height:clamp(21rem,44vw,31rem);overflow:auto;padding:1.4rem;border-top:1px solid var(--line);background:var(--diagram-bg);overscroll-behavior:contain;scrollbar-gutter:stable;touch-action:none;cursor:grab}.diagram-viewport:active{cursor:grabbing}.diagram-canvas{display:flex;align-items:center;width:100%;min-width:0;min-height:18rem;transform:none}.diagram-canvas svg{display:block;flex:none;height:auto;max-height:none;margin-inline:auto;overflow:visible;font-family:"Atkinson Hyperlegible Next",sans-serif}.diagram-loading{position:absolute;place-self:center;color:var(--faint);font-family:"Monaspace Neon",monospace;font-size:.9rem}.diagram-source{padding:.65rem .95rem;border-top:1px solid var(--line);color:var(--muted);font-size:.875rem}.diagram-source summary{display:flex;align-items:center;min-height:2.75rem;cursor:pointer}.diagram-source pre{max-height:22rem;margin:.7rem 0 0;padding:1rem;border-radius:.45rem;background:var(--code);color:var(--code-ink);font:400 .9375rem/1.68 "Monaspace Neon",monospace;overflow:auto}.diagram-error{place-self:stretch;padding:1.4rem;color:var(--risk);background:color-mix(in srgb,var(--risk) 7%,var(--diagram-bg))}.diagram-error p{margin:.4rem 0 0;color:var(--muted)}
...
.chart-viewport{max-width:100%;overflow:auto;padding:.9rem;border-top:1px solid var(--line);background:var(--diagram-bg)}.chart-viewport svg{display:block;width:100%;height:auto}.chart-grid line{stroke:var(--line)}.chart-grid text,.chart-label{fill:var(--muted);font-family:"Atkinson Hyperlegible Next",sans-serif}.chart-bar{fill:var(--accent)}.chart-line{fill:none;stroke:var(--accent);stroke-width:4;stroke-linecap:round;stroke-linejoin:round}.chart-dot{fill:var(--surface);stroke:var(--accent);stroke-width:3}.chart-data-table,.data-table{width:100%;border-collapse:collapse;font-size:.95rem}.chart-data-table caption{padding:.7rem .9rem;color:var(--muted);text-align:left}.chart-data-table th,.chart-data-table td,.data-table th,.data-table td{padding:.7rem .8rem;border-top:1px solid var(--line);text-align:left}.chart-data-table th,.data-table th{color:var(--muted);font-weight:650}.chart-data-table tbody th{color:var(--ink)}.table-scroll{max-width:100%;min-width:0;overflow-x:auto;overflow-y:hidden;overscroll-behavior-inline:contain;scrollbar-gutter:stable;contain:inline-size}.data-table{width:max-content;min-width:100%;max-width:none}.data-table th,.data-table td{min-width:11rem;max-width:25rem;vertical-align:top;overflow-wrap:anywhere}.data-table thead{position:sticky;top:0;background:var(--surface)}
...
.callout{max-width:70ch;margin:1.5rem 0;padding:.85rem 1rem;border-width:0 0 0 1px;border-style:solid;border-color:var(--line);background:transparent}.callout strong{color:var(--muted);font-size:.76rem;font-weight:650}.callout p{margin:.3rem 0 0}.callout-warning,.callout-important{border-color:var(--warn)}.callout-risk{border-color:var(--risk)}.callout-decision{border-color:var(--accent)}.render-error{margin:1.5rem 0;padding:1rem;border:1px solid var(--warn);border-radius:.55rem;background:var(--surface-2)}.render-error p{margin:.3rem 0}.mobile-reader-tools{display:none}.mobile-context{min-width:0}.mobile-context>.rail-action{margin-top:0}.mobile-reader-links{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.5rem;margin-top:1rem;padding-top:1rem;border-top:1px solid var(--line)}.mobile-reader-links .source-button{justify-content:center;border:1px solid var(--line)}.empty-document{padding:1.25rem;border:1px dashed var(--line);color:var(--muted)}
@media(max-width:82rem){body{padding-bottom:calc(4.5rem + env(safe-area-inset-bottom))}.reader-layout{display:block;max-width:none;padding:0}.document-column{max-width:60rem;margin:0 auto;padding:3rem clamp(1rem,5vw,3rem) 6rem}.document-column::before{inset:1rem 0 2rem}.artifacts-rail,.toc-rail{display:none}.reader-actions>.report-state,.reader-actions>.authority-state,.reader-actions>.source-button{display:none}.mobile-reader-tools{position:fixed;z-index:60;right:0;bottom:0;left:0;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));padding-bottom:env(safe-area-inset-bottom);border-top:1px solid var(--line);background:var(--document)}.mobile-reader-tools details{position:relative;min-width:0}.mobile-reader-tools summary{display:flex;align-items:center;justify-content:center;min-width:0;min-height:4rem;padding:.6rem;color:var(--muted);font-size:.95rem;text-align:center;cursor:pointer;list-style:none}.mobile-reader-tools summary::-webkit-details-marker{display:none}.mobile-reader-tools details[open] summary{color:var(--accent);background:var(--accent-soft)}.mobile-panel{position:fixed;right:50%;bottom:calc(4.75rem + env(safe-area-inset-bottom));width:min(calc(100% - 1.5rem),42rem);max-height:min(72dvh,38rem);overflow:auto;padding:1.1rem;border:1px solid var(--line);border-radius:.7rem;background:var(--document);box-shadow:var(--shadow);transform:translateX(50%);overscroll-behavior:contain}.mobile-panel .artifact-link{grid-template-columns:1.75rem 1fr}.mobile-panel .toc-link{padding:.65rem}.document-meta{grid-template-columns:none}}
@media(max-width:58rem){.visual-block figcaption,.code-block figcaption{align-items:flex-start;flex-direction:column}.visual-title{width:100%;flex-wrap:wrap}.visual-actions{justify-content:flex-start;width:100%}.visual-actions button{flex:1 1 6.5rem}}
@media(max-width:42rem){.reader-topbar{padding:.55rem .75rem}.reader-identity strong{max-width:none}.reader-identity>span:last-child{display:none}.document-column{padding:2.25rem 1rem 5.75rem}.document-column::before{inset:.5rem 0 1rem;border-radius:.65rem}h1{font-size:clamp(2.05rem,8vw,2.45rem)}.document-summary{font-size:1.15rem}.document-meta{gap:.6rem 1.1rem}.visual-block figcaption,.code-block figcaption{align-items:flex-start;flex-direction:column}.visual-actions{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));justify-content:stretch;width:100%}.visual-actions button{min-width:0;padding:.35rem .15rem;font-size:.8rem}.diagram-viewport{min-height:22rem;padding:.75rem}.mermaid-block.is-expanded{inset:.35rem}.mobile-panel{width:calc(100% - .75rem);padding:1rem}.mobile-reader-links{grid-template-columns:1fr}.chart-data-table th,.chart-data-table td{padding:.65rem}.table-scroll{overflow:visible;contain:none}.data-table{display:block;width:100%;min-width:0}.data-table thead{position:absolute;width:1px;height:1px;margin:-1px;padding:0;overflow:hidden;clip:rect(0 0 0 0);white-space:nowrap;border:0}.data-table tbody,.data-table tr{display:block;width:100%;min-width:0}.data-table tr{padding:.3rem 0;border-top:1px solid var(--line)}.data-table tr:first-child{border-top:0}.data-table td{display:grid;grid-template-columns:minmax(6.5rem,35%) minmax(0,1fr);gap:.75rem;width:100%;min-width:0;max-width:none;padding:.65rem;border:0;text-align:left!important;overflow-wrap:anywhere}.data-table td::before{content:attr(data-label);color:var(--muted);font-size:.78rem;font-weight:650;line-height:1.4}}
...
<head>
  ...
  <style>${readerCss}${isReport ? reportCss : ""}</style>
</head>
<body${isReport ? ' class="reader-report"' : ""}>
  <a class="skip-link" href="#document">${language === "es" ? "Ir al documento" : "Skip to document"}</a>
  <header class="reader-topbar">
    <div class="reader-identity">${isReport ? '<svg class="report-mark" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true"><path d="M4 3h11l5 5v13H4zM15 3v6h5M8 12h8M8 16h6"/></svg>' : '<span class="reader-mark" aria-hidden="true">WB</span>'}<strong>${escapeHtml(workflow.name || model.productName)}</strong><span>${escapeHtml(document.type)}</span></div>
    <div class="reader-actions">${reportState}${isReport ? "" : `<span class="authority-state${workflow.implementationAuthorized === true ? " is-authorized" : ""}" title="${escapeHtml(authorizationLabel)}">${escapeHtml(authorizationLabel)}</span>`}${libraryButton}${sourceButton}<button class="icon-button" type="button" data-theme-toggle aria-label="${language === "es" ? "Cambiar tema claro u oscuro" : "Toggle light and dark theme"}" aria-pressed="false"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true"><path d="M20.5 14.2A8.6 8.6 0 0 1 9.8 3.5a8.7 8.7 0 1 0 10.7 10.7Z"/></svg></button></div>
  </header>
  <main class="reader-layout">
    <article class="document-column" id="document">
      <header class="document-header">
        ${isReport ? "" : `<div class="document-kicker"><span>${escapeHtml(document.type)}</span><span class="status-chip">${escapeHtml(document.status)}</span>${document.priority ? `<span class="priority-chip">${escapeHtml(labels.priority)} · ${escapeHtml(document.priority)}</span>` : ""}</div>`}
        <h1>${escapeHtml(document.title)}</h1>
        ${document.summary ? `<p class="document-summary">${inlineMarkdown(text(document.summary))}</p>` : ""}
        ${isReport ? `<p class="report-document-state">${escapeHtml(document.type)}<span aria-hidden="true"> · </span>${escapeHtml(document.status)}</p>` : ""}
    ...
    ${isReport ? "" : `<aside class="artifacts-rail" aria-label="${escapeHtml(labels.artifacts)}"><p class="rail-label"><span>${escapeHtml(labels.artifacts)}</span><span>${escapeHtml(document.profile ?? "")}</span></p><ol class="artifact-list">${artifactItems || '<li class="artifact-link">No artifacts yet</li>'}</ol>${workflow.action ? `<div class="rail-action"><span>${escapeHtml(labels.nextAction)}</span><strong>${escapeHtml(workflow.action)}</strong></div>` : ""}</aside>`}
    <aside class="toc-rail" aria-label="${escapeHtml(labels.onThisPage)}"><p class="rail-label">${escapeHtml(labels.onThisPage)}</p><ol class="toc-list">${tocItems || '<li class="toc-link">No sections yet</li>'}</ol>${isReport ? "" : `<div class="gate-card"><span>${escapeHtml(gateLabel)}</span><strong>${escapeHtml(document.type)}</strong>${gateReportState}<span class="authorization-label">${escapeHtml(authorizationLabel)}</span></div>`}</aside>
  </main>
  <nav class="mobile-reader-tools" aria-label="Reader navigation" data-mobile-reader>${isReport ? "" : `<details name="reader-navigation" data-mobile-details><summary>${escapeHtml(labels.artifacts)}</summary><div class="mobile-panel"><ol class="artifact-list">${artifactItems || '<li class="artifact-link">No artifacts yet</li>'}</ol></div></details>`}<details name="reader-navigation" data-mobile-details><summary>${escapeHtml(labels.onThisPage)}</summary><div class="mobile-panel"><ol class="toc-list">${tocItems || '<li class="toc-link">No sections yet</li>'}</ol></div></details><details name="reader-navigation" data-mobile-details><summary>${escapeHtml(labels.details)}</summary><div class="mobile-panel">${mobileContext}</div></details></nav>
