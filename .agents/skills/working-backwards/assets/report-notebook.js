(() => {
  ...
  document.addEventListener("keydown", (event) => {
    ...
    if (event.key === "Tab" && expanded) {
      const controls = [
        ...expanded.querySelectorAll('button,summary,[tabindex="0"]'),
  ...
  reduced.addEventListener("change", () => {
    if (reduced.matches)
      document
        .querySelectorAll('[data-map-motion][aria-pressed="true"]')
  ...
  document.querySelectorAll(".document-nav a").forEach((link) =>
    link.addEventListener("click", () => {
      document.querySelector(".document-nav").open = false;
    }),
  );
  ...
  const mark = () => {
    ...
    if (requested)
      active =
        headings.find((heading) => "#" + heading.id === requested) || active;
  ...
  const t = es
    ? {
        ask: "Preguntar sobre este párrafo",
        label: "Tu pregunta sobre el párrafo seleccionado",
        ...
        sent: "Enviada",
        count: (n) =>
          n === 1 ? "1 pregunta guardada" : `${n} preguntas guardadas`,
        empty: "Preguntar sobre un párrafo",
        ...
        required: "Escribe la pregunta antes de guardarla.",
        stored: "Pregunta guardada en este navegador.",
        deleted: "Pregunta borrada.",
        noStorage:
          "Este navegador no permite guardar borradores. Copia tus preguntas antes de cerrar.",
        sending: "Enviando tus preguntas…",
        delivered: (receipt) =>
          `Preguntas enviadas. Vuelve al chat y di «ya pregunté». Recibo: ${receipt}`,
        conflict:
          "Otra pestaña envió preguntas antes. Conservamos las tuyas; vuelve a enviar para crear una revisión nueva.",
        offline:
          "Este archivo se abrió sin el servidor local. Copia las preguntas y pégalas en el chat, o descárgalas.",
        copied: "Copiadas. Pégalas en el chat con el modelo.",
        copyFailed: "No se pudo copiar. Descarga el archivo JSON.",
        picking:
          "Usa Tab para recorrer los párrafos y Enter para preguntar. Escape termina.",
        about: "Sobre",
        header: "Encabezado",
        findings: "Hallazgos",
        intro: (title, n) => `Preguntas sobre «${title}» (${n})`,
        question: "Pregunta",
      }
    : {
        ask: "Ask about this paragraph",
        label: "Your question about the selected paragraph",
        ...
        saved: "Saved",
        sent: "Sent",
        count: (n) => (n === 1 ? "1 saved question" : `${n} saved questions`),
        empty: "Ask about a paragraph",
        send: "Send to the model",
        ...
        required: "Write the question before saving it.",
        stored: "Question saved in this browser.",
        deleted: "Question deleted.",
        noStorage:
          "This browser cannot keep drafts. Copy your questions before closing.",
        sending: "Sending your questions…",
        delivered: (receipt) =>
          `Questions sent. Return to the chat and say “questions sent”. Receipt: ${receipt}`,
        conflict:
          "Another tab sent questions first. Yours are kept; send again to create a new revision.",
        offline:
          "This file was opened without the local server. Copy the questions into the chat, or download them.",
        copied: "Copied. Paste them into the chat with the model.",
        copyFailed: "Copy failed. Download the JSON file instead.",
        picking:
          "Use Tab to move through paragraphs and Enter to ask. Escape ends.",
        about: "About",
        header: "Header",
        findings: "Findings",
        intro: (title, n) => `Questions about “${title}” (${n})`,
        question: "Question",
      };
  ...
  const endpoint = /^https?:$/.test(location.protocol)
    ? location.pathname.replace(/\.html?$/i, "") + ".questions.json"
    : null;
  const title =
    (document.querySelector(".brief-header h1") || {}).textContent ||
    document.title;
  ...
  try {
    ...
    if (saved && Array.isArray(saved.questions))
      state = {
        questions: saved.questions.filter(
          (item) => item && typeof item.question === "string",
        ),
        serverRevision: Number(saved.serverRevision) || 0,
        sentSignature: String(saved.sentSignature || ""),
      };
  } catch {
    /* A corrupt draft is ignored rather than blocking the report. */
  }
  const signature = (questions) =>
    JSON.stringify(
      questions.map((item) => [item.id, item.blockId, item.question]),
    );
  const persist = () => {
    if (!store.set(key, JSON.stringify(state))) announce(t.noStorage, "error");
  };
  const icon = (path) =>
  ...
  const sectionOf = (block) => {
    ...
    const heading =
      block.closest(".brief-section") &&
      block.closest(".brief-section").querySelector("h2");
    return heading ? heading.textContent.replace(/^\d{2}/, "").trim() : "";
  };
  const marginVisible = () =>
    margin.getClientRects().length > 0 &&
    getComputedStyle(margin).display !== "none" &&
    margin.offsetWidth > 120;
  const topOf = (block) =>
    block.getBoundingClientRect().top - margin.getBoundingClientRect().top;
  const ordered = () =>
  ...
  function openComposer(block, question) {
    ...
    if (!marginVisible()) {
      block.scrollIntoView({
        block: "start",
        behavior: reduced.matches ? "auto" : "instant",
      });
      ...
      if (barBottom) {
        ...
        if (heading) {
          ...
          if (rect.top < barBottom && rect.bottom > barBottom)
            scrollBy({ top: rect.bottom - barBottom + 16, behavior: smooth });
        }
      }
      return;
    }
    const overflow =
      composer.getBoundingClientRect().bottom - (innerHeight - 110);
  ...
  function stopPicking() {
    ...
    document
      .querySelectorAll('[data-q][tabindex="0"][data-picking]')
      .forEach((block) => {
        block.removeAttribute("tabindex");
        block.removeAttribute("data-picking");
      });
  }
  function startPicking() {
    root.dataset.picking = "true";
    const blocks = [...document.querySelectorAll("[data-q]")].filter(
      (block) => block.getClientRects().length,
    );
    blocks.forEach((block) => {
      if (!block.hasAttribute("tabindex")) {
        block.tabIndex = 0;
        block.dataset.picking = "";
      }
    });
    panel.hidden = true;
    renderTray();
    const visible =
      blocks.find((block) => block.getBoundingClientRect().top > 80) ||
      blocks[0];
  ...
  function save(event) {
    ...
    if (!text) {
      composer.querySelector(".ask-error").textContent = t.required;
      input.focus();
      return;
    }
    ...
    if (editing) Object.assign(editing, { question: text, updatedAt: now });
      ...
      state.questions.push({
        ...
        blockId: active.dataset.q,
        section: sectionOf(active),
        excerpt: excerptOf(active),
        question: text,
        createdAt: now,
        updatedAt: now,
      });
  ...
  function noteItem(question, index, withExcerpt) {
    ...
    const sent =
      state.sentSignature && state.sentSignature === signature(state.questions);
    ...
    item.addEventListener("mouseenter", () => {
      const block = blockFor(question.blockId);
      if (block) block.classList.add("is-linked");
    });
    item.addEventListener("mouseleave", () => {
      const block = blockFor(question.blockId);
      if (block) block.classList.remove("is-linked");
    });
    return item;
  }
  function renderTray() {
    ...
    const sent =
      count > 0 && state.sentSignature === signature(state.questions);
    ...
    open.addEventListener("click", () => {
      ...
      renderTray();
      if (!panel.hidden) panel.querySelector(".panel-close").focus();
    });
    tray.append(open);
  ...
  function render() {
    ...
    document
      .querySelectorAll("[data-q][data-asked]")
      .forEach((block) => block.removeAttribute("data-asked"));
    questions.forEach((question) => {
      const block = blockFor(question.blockId);
      if (block) block.dataset.asked = "";
    });
    notesList.replaceChildren(
      ...questions.map((question, index) => noteItem(question, index, false)),
    );
    panel
      .querySelector(".panel-list")
      .replaceChildren(
        ...questions.map((question, index) => noteItem(question, index, true)),
      );
    renderTray();
    layout();
  }
  ...
  function exportText() {
    const questions = ordered();
    return [
      t.intro(title.trim(), questions.length),
      ...questions.map(
        (question, index) =>
  ...
  composer
    ...
    .addEventListener("click", () => closeComposer(true));
  ...
  pill.addEventListener("click", () => {
    if (active) openComposer(active);
  });
  panel.querySelector(".panel-close").addEventListener("click", () => {
    panel.hidden = true;
    renderTray();
    tray.querySelector(".tray-open").focus();
  });
  ...
  document.addEventListener("click", (event) => {
    ...
    if (
      ...
      target.closest(
        ".ask-composer,.ask-pill,.question-tray,.question-panel,.margin-notes",
      )
    )
      return;
    if (
      target.closest(
        "a,button,summary,input,textarea,select,video,label,.map-scroll,.document-sidebar,.brief-topbar,.brief-footer",
      )
    )
      return;
  ...
  document.addEventListener("keydown", (event) => {
    const block =
      event.target instanceof Element && event.target.matches("[data-q]")
        ? event.target
        : null;
  ...
  if ("ResizeObserver" in window)
    new ResizeObserver(() => layout()).observe(
      document.querySelector(".brief") || document.body,
    );
    ...
    fetch(endpoint, { signal: AbortSignal.timeout(5000) })
      ...
      .then((saved) => {
        ...
        const serverQuestions =
          saved.data && Array.isArray(saved.data.questions)
        ...
        if (!state.questions.length && serverQuestions.length) {
          ...
          signature(serverQuestions) === signature(state.questions)
        ) {
