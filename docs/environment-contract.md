...
PostHog is production-only. Local development and every preview deployment omit `PUBLIC_POSTHOG_KEY` and emit zero events. The retained production project key is public by design, but the browser SDK and server reporters send through the canonical-domain first-party `/ingest` proxy. The proxy rejects non-production environments and non-canonical hosts. Autocapture, automatic pageview/pageleave capture, persistence, person profiles, and session recording are disabled; Do Not Track is respected. `$pageview`, `$pageleave`, `$web_vitals`, selected conversions, safe friction signals, dashboard actions, and fixed-shape exceptions are emitted manually after sanitization.
...
| `$web_vitals` | Public and dashboard browser clients | Sanitized page context plus fixed metric name and numeric value |
