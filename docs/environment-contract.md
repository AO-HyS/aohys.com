...
| ------------------ | ------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| Public build value | Safe to expose to browser bundles                                   | public site URL, production-only PostHog public key                         |
...
PostHog is production-only. Local development and every preview deployment omit `PUBLIC_POSTHOG_KEY` and emit zero events. The retained production project key is public by design, but the browser SDK and server reporters send through the canonical-domain first-party `/ingest` proxy. The proxy rejects non-production environments and non-canonical hosts. DOM autocapture, automatic pageview/pageleave capture, person profiles, session recording, surveys, tours, and dead-click capture are disabled; Do Not Track is respected. The public browser keeps an anonymous first-party identifier in local storage so users and sessions remain coherent across page loads. `$pageview`, `$pageleave`, selected conversions, safe friction signals, dashboard actions, and fixed-shape exceptions are explicit and sanitized; the SDK captures only real LCP, INP, and CLS `$web_vitals` measurements.

During setup or incident recovery, verify that GitHub Environment and Cloudflare Pages preview do not define `PUBLIC_POSTHOG_KEY`, while production defines the retained production project key. The Release Train audits this before deployment and smoke-checks that preview `/ingest` is closed while production serves it.

Current explicit PostHog events:

| Event                           | Emitted by                                               | Privacy rule                                                                                                                                                 |
| ------------------------------- | -------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `$pageview`                     | Astro public layout browser client                       | Fixed page context plus PostHog browser/device, coarse country, anonymous session, and virtual-traffic classification; no query string or contact identity   |
| `$pageleave`                    | Public and dashboard browser clients                     | Same sanitized page context; captured manually once when hidden                                                                                              |
| `$web_vitals`                   | PostHog public browser SDK and explicit dashboard client | Public SDK emits only real LCP, INP, and CLS properties; dashboard events retain their sanitized fixed metric contract                                       |
| `$exception`                    | Public browser client                                    | Fixed source and error type only; no SDK stack, message, URL query, or identity                                                                              |
...
| `lead_intake_rejected`          | Convex contact HTTP boundary                             | Expected malformed, invalid, abusive, or rate-limited reason plus safe request-shape metadata; no message text or contact identity                           |
| `lead_intake_failed`            | Convex contact HTTP boundary                             | Real backend failure reason, environment, public error code/status, safe route context, and boolean presence flags only; no message text or contact identity |
...
| PostHog     | retained production public key and event inspection; the app owns the first-party proxy and disabled capture policies |
