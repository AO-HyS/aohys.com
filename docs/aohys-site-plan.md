...
- Public site refinement status: the June 30 polish pass replaced the light collage hero with a dark architecture proof board, rewrote the home and primary case-study copy to avoid internal/robotic phrasing, added distinct evidence assets per case, and made external evidence links open actual public or development pages when one exists.
