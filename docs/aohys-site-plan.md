...
15. Add dashboard, Better Auth, media management, and private workflows after the public shell proves the design and content direction. Current status: first dashboard guard/shell implemented with Cloudflare Pages functions, Dashboard UI Kit renderers, server-side Better Auth Google sign-in bridge through Convex, admin allowlist checks, noindex/no-store responses, and auth-specific Environment Contract targets. Lead/media/content workflows remain next slices.
