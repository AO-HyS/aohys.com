...
4. GitHub acknowledgement only moves the attempt to `acknowledged`. A workflow-level reconciliation job records correlated Release Train plan/deploy failures, plus cancellation paths when that downstream job starts, as explicit terminal evidence and retryable `release-failed`. The Release Train records an internal `PublicationReceipt` only after the environment smoke passes, and that receipt is the only transition to `deployed`.
...
- A Release Train failure/cancellation callback is idempotent only when request, attempt, target, branch, run id/URL, release SHA, and outcome all match persisted evidence. Even after deployment, any mismatch fails closed before the callback becomes a no-op. A later dashboard retry is allowed only after terminal evidence (or a safe `not-found` reconciliation), never merely because time elapsed.
...
The reconciliation job depends on release planning and both target deploy jobs with `if: always()`, then inspects the relevant `needs.*.result`. This automatically covers ordinary plan/deploy failures. GitHub's [workflow cancellation reference](https://docs.github.com/en/actions/reference/workflows-and-actions/workflow-cancellation) only guarantees condition re-evaluation for jobs that are already running; it does not guarantee that this pending downstream `needs` job will start after a concurrency or UI cancellation. If it does start, it records the cancellation. Otherwise, inspect the correlated GitHub run first and run `pnpm exec tsx scripts/record-publication-outcome.ts` manually with the original request key, attempt id, target, Git ref, run id/URL, release SHA, terminal outcome, and environment `CONVEX_DEPLOY_KEY`. A force-cancel, runner outage, or failure inside reconciliation uses the same manual path. If GitHub cannot prove a terminal run, use the stale-claim status-check path and record `unknown` as `rollback-needed`; never retry from elapsed time alone.
