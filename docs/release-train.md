...
| `pnpm run audit:posthog-env`                   | Verify preview has no PostHog key and production has the retained project key; in Actions validate only the injected target environment.                                                                                                                           |
...
| `pnpm run deploy:preview`                      | Validate preview env, audit the zero-telemetry PostHog boundary, sync Convex preview runtime variables, deploy Convex with the preview deploy key, seed preview dashboard drafts/settings, apply published dashboard content, build `apps/site`, and deploy the `develop` branch. |
| `pnpm run deploy:production`                   | Validate production env, audit the production-only PostHog key, sync Convex production runtime variables, deploy Convex with the production deploy key, apply published dashboard content, build `apps/site`, deploy `main`, and reconcile the canonical domain before smoke checks.                                          |
...
| `pnpm run smoke:preview`                       | Fetch the preview smoke URL, verify a 2xx public shell, production canonical URL, closed `/ingest` proxy, Convex CSP allowances, anonymous dashboard redirect, private sign-in shell, and contact endpoint.                                                        |
...
- a self-only PostHog CSP path and Convex allowances, without third-party PostHog origins;
- contact page endpoint checks before promotion;
- no preview PostHog key and one required production key before deploy;
