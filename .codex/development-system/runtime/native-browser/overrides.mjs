...
export async function nativeBrowserOverrides(paths = DEFAULT_PATHS) {
  for (const name of /** @type {const} */ (["node", "repl", "nodeRepl"])) {
    ...
    await access(
      paths[name],
      name === "repl" ? constants.R_OK : constants.X_OK,
    );
  }
  return [
    "-c",
    `mcp_servers.node_repl.command=${JSON.stringify(paths.node)}`,
    "-c",
    `mcp_servers.node_repl.args=${JSON.stringify([paths.repl])}`,
    "-c",
    'mcp_servers.node_repl.env.CUA_REPL_ENABLED_SURFACES="browser,computer"',
    "-c",
    `mcp_servers.node_repl.env.CUA_REPL_NODE_REPL_PATH=${JSON.stringify(paths.nodeRepl)}`,
  ];
}
