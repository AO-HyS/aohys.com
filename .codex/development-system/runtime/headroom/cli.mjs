...
const FLAGS = [
  ...
  "--no-telemetry",
  "--compressor",
  "smart_crusher,search,log,tabular,config,html",
];
...
function getJson(port, endpoint, timeout = 1500) {
  return new Promise((resolve, reject) => {
    const request = http.get(
      { hostname: "127.0.0.1", port, path: `/${endpoint}`, timeout },
      (response) => {
        if (response.statusCode !== 200) {
          response.resume();
          reject(new Error(`${endpoint}: HTTP ${response.statusCode}`));
          return;
        }
        let body = "";
    ...
    request.on("timeout", () =>
      request.destroy(new Error(`${endpoint}: timeout`)),
    );
...
function numericStats(value, depth = 0) {
  ...
  for (const [key, item] of Object.entries(value)) {
    if (
      !/token|request|cache|cost|saved|hit|miss|count|total|error|latency/i.test(
        key,
      )
    )
      continue;
    if (typeof item === "number" && Number.isFinite(item)) stats[key] = item;
      ...
      stats[key] = numericStats(item, depth + 1);
  }
  return stats;
}
...
function requestedModels(args) {
  ...
  for (let index = 0; index < args.length; index++) {
    ...
    else if (arg === "-c" && /^model=/.test(args[index + 1] ?? ""))
      models.push(args[++index].slice(6).replace(/^['"]|['"]$/g, ""));
  }
  return models;
}
...
function rejectTransportOverrides(args) {
  for (let index = 0; index < args.length; index++) {
    ...
    if (/^--(?:oss|local-provider|remote)(?:=|$)/.test(arg)) {
      throw new Error(
        "caller cannot select a transport outside the Headroom provider",
      );
    }
    const value =
      arg === "-c" || arg === "--config"
    ...
    if (
      ...
      throw new Error(
        "caller cannot override the per-invocation Headroom provider",
      );
    }
  }
}
...
async function main() {
  ...
  if (
    ...
    throw new Error(
      "usage: node runtime/headroom/cli.mjs --config /private/config.json -- <codex arguments>",
    );
  }
  ...
  const passthrough =
    ...
    (args.length === 2 &&
      ["--version", "-V", "--help", "-h"].includes(args[1]));
  await access(codex, constants.X_OK);
  await access(root, constants.R_OK);
  await access(codexHome, constants.R_OK);
  for (const model of requestedModels(args)) {
    if (!ALLOWED_MODELS.has(model))
      throw new Error(`unsupported configured model: ${model}`);
  }
  if (config.model !== undefined && !ALLOWED_MODELS.has(config.model))
    throw new Error(`unsupported configured model: ${config.model}`);
  ...
  const beginShutdown = () => {
    shutdownTask ??= Promise.all([
      stopOwned(codexChild, codexDone),
      stopOwned(proxy, proxyDone),
    ]);
  ...
  try {
    if (!passthrough) {
      ...
      proxy = spawn(
        headroom,
        ["proxy", "--host", "127.0.0.1", "--port", String(port), ...FLAGS],
        {
          cwd: root,
          env: proxyEnvironment(path.join(receipt, "state"), codexHome),
          detached: true,
      ...
      proxyDone.then(
        () => {
          if (!closing && !interrupted) {
            proxyFailure = "Headroom proxy exited unexpectedly";
            beginShutdown().catch(() => {});
          }
        },
        () => {
          if (!closing && !interrupted) {
            proxyFailure = "Headroom proxy failed unexpectedly";
            beginShutdown().catch(() => {});
          }
        },
      );
      ...
      if (!ready)
        throw new Error(`Headroom health unavailable; private log: ${logPath}`);
      try {
        await writePrivate(
          path.join(receipt, "proxy-before.json"),
          numericStats(await getJson(port, "stats")),
        );
      } catch {
        await writePrivate(path.join(receipt, "proxy-before.json"), {
          unavailable: true,
        });
      }
    }
    const providerOverrides = passthrough
      ? []
      : [
          "-c",
          'model_provider="headroom_local"',
          "-c",
          `model_providers.headroom_local={name="Headroom local",base_url="http://127.0.0.1:${port}/v1",wire_api="responses",requires_openai_auth=true,supports_websockets=true}`,
          ...(config.nativeBrowser
            ? await nativeBrowserOverrides(
                config.nativeBrowser === true
                  ? undefined
                  : config.nativeBrowser,
              )
            : []),
    ...
    const commandArgs = passthrough
      ...
      : args[0] === "app-server"
        ...
        : ["--no-daemon", "-C", root, ...providerOverrides, ...args];
    if (receipt)
      await writePrivate(path.join(receipt, "launch.json"), {
        ...
        port,
        codexBinary: codex,
        headroomBinary: config.headroomBinary,
        requestedModel:
          requestedModels(args).at(-1) ?? config.model ?? "caller-selected",
        observedModel: "unknown",
        nativeTokenUsage: "not collected by wrapper",
        browserReadiness: "not observed",
      });
    if (interrupted || proxyFailure)
      throw new Error(proxyFailure ?? `interrupted by ${interrupted}`);
    codexChild = spawn(codex, commandArgs, {
      cwd: root,
    ...
    if (receipt)
      await writePrivate(path.join(receipt, "exit.json"), {
        closedAt: stamp(),
        exitCode: result.code,
        signal: result.signal,
      });
    process.exitCode = proxyFailure
      ? 1
      : interrupted
        ? 128 + (os.constants.signals[interrupted] ?? 1)
        : (result.code ??
          (result.signal
    ...
    if (
      ...
      try {
        await writePrivate(
          path.join(receipt, "proxy-after.json"),
          numericStats(await getJson(port, "stats")),
        );
      } catch {
        await writePrivate(path.join(receipt, "proxy-after.json"), {
          unavailable: true,
        });
      }
...
main().catch((error) => {
  console.error(`headroom launcher: ${error.message}`);
  process.exitCode = 1;
});
