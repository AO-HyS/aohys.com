...
function getJson(port, endpoint) {
  return new Promise((resolve, reject) => {
    const request = http.get(
      { hostname: "127.0.0.1", port, path: `/${endpoint}`, timeout: 1500 },
      (response) => {
...
function signalGroup(child, signal) {
  ...
  try {
    process.kill(grouped.has(child) ? -child.pid : child.pid, signal);
    return true;
  } catch (error) {
...
async function main() {
  ...
  if (separator < 0 || options.length !== 2 || options[0] !== "--config") {
    throw new Error(
      "usage: node runtime/headroom/claude.mjs --config /private/config.json -- <claude arguments>",
    );
  }
  ...
  const passthrough =
    (args.length === 1 && PASSTHROUGH.has(args[0])) ||
    MANAGEMENT.has(args[0] ?? "");
  ...
  if (!passthrough) {
    for (const key of [
      ...
      if (process.env[key])
        throw new Error(
          `${key} is set; refusing to replace another Claude transport`,
        );
    }
  }
  ...
  const beginShutdown = () => {
    shutdownTask ??= Promise.all([
      stopOwned(claudeChild, claudeDone),
  ...
  try {
    ...
    if (!passthrough) {
      ...
      proxy = spawn(
        ...
        {
          cwd: receipt,
          env: proxyEnvironment(path.join(receipt, "state")),
          detached: true,
      ...
      await writePrivate(path.join(receipt, "launch.json"), {
        ...
        port,
        claudeBinary: claude,
        headroomBinary: headroom,
        requestedModel: requestedModel(args) ?? "caller-selected",
    ...
    if (proxyFailure)
      console.error(
        `headroom claude launcher: ${proxyFailure}; private receipt: ${receipt}`,
      );
    ...
    if (receipt && port) {
      ...
      if (alive) {
        try {
          after = numericStats(await getJson(port, "stats"));
        } catch {
...
main().catch((error) => {
  console.error(`headroom claude launcher: ${error.message}`);
  process.exitCode = 1;
});
