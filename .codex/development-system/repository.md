...
Repository adapter readiness is structural, not proof of skill loading. Synchronize global skill catalog `0.20.0` and verify that Codex discovers these commands plus `drive-development-flow`, `coding-orchestration`, `working-backwards`, `parallel-work`, `simplify-code`, `orchestration-pilot`, and `check-in`. The internal `orchestration-plan` CLI operation is consumed by `coding-orchestration`; it is not a `# Development System repository adapter
...
Repository adapter readiness is structural, not proof of skill loading. skill. T3 Code consumes the Codex-compatible surface. T3 Code shares the Codex adapter structurally but has no independent live command proof in this release.
