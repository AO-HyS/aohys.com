...
export const FLASH_PROFILE = Object.freeze({
...
export function workerEnvironment(environment = process.env) {
  return Object.fromEntries(
    Object.entries(environment).filter(
      ([key]) =>
        !/^(?:TYPESAFE|JEV|AOHYS_JEV)/iu.test(key) &&
        ![
          "NODE_OPTIONS",
          "NODE_PATH",
          "BASH_ENV",
...
export function observedOpenCodeSession(stdout) {
  ...
  for (const line of stdout.split("\n")) {
    ...
    if (typeof id === "string" && /^ses_[A-Za-z0-9]+$/u.test(id))
      sessions.add(id);
  }
  if (sessions.size !== 1)
    throw new Error("OpenCode did not emit one attributable session.");
...
export async function collectOpenCodeObservation({
  executable,
  sessionId,
  candidateRoot,
  env,
  ...
  try {
    await new Promise((done, reject) => {
      const child = spawn(executable, ["export", sessionId, "--pure"], {
        cwd: candidateRoot,
        env,
        shell: false,
      ...
      child.once("close", (code) => {
        clearTimeout(timer);
        if (code === 0) done(undefined);
        else reject(new Error("OpenCode export failed"));
      });
    });
    if ((await file.stat()).size > 16 * 1024 * 1024)
      throw new Error("OpenCode export exceeded its private spool cap");
  ...
  if (exported.info?.id !== sessionId || !Array.isArray(exported.messages))
    throw new Error("OpenCode export session provenance is unavailable.");
  const assistants = exported.messages.filter(
    (/** @type {any} */ message) => message.info?.role === "assistant",
  );
  if (!assistants.length)
    throw new Error("OpenCode export has no observed assistant identity.");
  for (const message of assistants) {
    const info = message.info;
    if (
      info.sessionID !== sessionId ||
      info.providerID !== FLASH_PROFILE.provider ||
      info.modelID !== "deepseek-v4.1-flash" ||
      info.variant !== FLASH_PROFILE.reasoning
    )
      throw new Error(
        "OpenCode served identity does not match the permitted profile.",
      );
  }
  const output = assistants
    .flatMap((/** @type {any} */ message) =>
      (message.parts ?? [])
