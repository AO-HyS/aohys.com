...
export function codexArguments({
  ...
  return [
    ...
    "--json",
    "--sandbox",
    role === "writer" ? "workspace-write" : "read-only",
    "--model",
    model,
    "-c",
    ...
    ...(role === "writer"
      ? WRITER_CONFIG.flatMap((config) => ["-c", config])
      : []),
    "--cd",
    candidateRoot,
...
export function matchesCodexCommand(
  argv,
  { role, model, reasoning, candidateRoot },
) {
  if (
    !Array.isArray(argv) ||
  ...
  for (let index = 1; index < argv.length; index++) {
    ...
    if (
      ...
      if (value === undefined || !value.length || value.startsWith("-"))
        return false;
      if (flag === "-c") {
    ...
    if (
      ...
      flag.startsWith("-") ||
      index !== argv.length - 1 ||
      ["resume", "fork", "review", "help"].includes(flag)
    )
      return false;
    prompt = flag;
  }
  if (
    ...
    options.get("--sandbox") !== (writer ? "workspace-write" : "read-only") ||
    !configs.has(effort) ||
    (tier && !configs.has(`service_tier="${tier}"`))
  )
    return false;
  if (options.has("--cd") && options.get("--cd") !== candidateRoot)
    return false;
  ...
  if (
    ...
    prompt === undefined ||
    !options.has("--json") ||
    options.get("--cd") !== candidateRoot ||
    configs.size !== allowedConfig.size
  )
    return false;
  const expected = codexArguments({
    role,
    model,
    reasoning,
    candidateRoot,
    schemaPath: /** @type {string|undefined} */ (
      options.get("--output-schema")
    ),
    imagePaths,
    prompt,
  });
...
export function allowsCodexWriterRequirements(result) {
  if (
    ...
    !Object.hasOwn(result, "requirements")
  )
    return false;
  ...
  if (
    ...
    return false;
  if (
    !["allowedPermissionProfiles", "defaultPermissions"].every(
      (key) => Object.hasOwn(requirements, key) && requirements[key] === null,
    )
  )
    return false;
  return [
    ...
    ([key, required]) =>
      !Object.hasOwn(requirements, key) ||
      requirements[key] === null ||
      (Array.isArray(requirements[key]) &&
        requirements[key].every(
          (/** @type {any} */ value) => typeof value === "string",
        ) &&
        requirements[key].includes(required)),
  );
}
...
export function matchesCodexWriterPermissions(metadata, candidateRoot) {
  ...
  if (
    ...
    sandbox.exclude_tmpdir_env_var !== true ||
    sandbox.exclude_slash_tmp !== true ||
    (Object.hasOwn(sandbox, "writable_roots") &&
      (!Array.isArray(sandbox.writable_roots) ||
        sandbox.writable_roots.length !== 0))
  )
    return false;
  ...
  if (
    ...
    !Array.isArray(profile.file_system.entries) ||
    Object.keys(profile).some(
      (key) => !["type", "network", "file_system"].includes(key),
    ) ||
    Object.keys(profile.file_system).some(
      (key) => !["type", "entries"].includes(key),
    )
  )
    return false;
  return profile.file_system.entries.every((/** @type {any} */ entry) => {
    if (
      ...
      !entry.path ||
      !["path", "special"].includes(entry.path.type)
    )
      return false;
    ...
    if (
      ...
      resolve(entry.path.path) !== entry.path.path
    )
      return false;
