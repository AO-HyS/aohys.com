...
import {
  bindProcessCandidate,
  buildObservationAssessmentInput,
  executionDescriptor,
  getRun,
  recordHostEvent,
...
import {
  assertRunRoots,
  hostRootFor,
  readProcessRootIdentity,
  readPrivateArtifact,
  repositoryDelta,
  snapshotRepository,
} from "./store.mjs";
import {
  collectOpenCodeObservation,
  flashArguments,
  FLASH_PROFILE,
  observedOpenCodeSession,
  workerEnvironment,
} from "./opencode.mjs";
import {
  allowsCodexWriterRequirements,
  codexArguments,
  matchesCodexWriterPermissions,
  requiredCodexServiceTier,
} from "./codex.mjs";
      ...
      role === "reviewer"
        ? (run.reviews
            ...
            .map((/** @type {any} */ review) => ({
              ...
              invalidated: review.invalidated === true,
            }))
            .at(-1) ?? null)
        : null,
    unresolvedFindings: run.findings.filter(
      (/** @type {any} */ finding) => !finding.resolved,
    ),
    observedChecks: run.verifications.map(
      (/** @type {any} */ verification) => ({
        id: verification.id,
        results: verification.results,
        command: verification.command,
        invalidated: verification.invalidated === true,
      }),
    ),
  };
}
...
export async function rejectedReviewContext(run, role, runDirectory) {
  ...
  const attempt = run.attempts
    .filter(
      (/** @type {any} */ item) =>
        item.role === role && item.status === "failed" && item.rejectedOutput,
    )
    .at(-1);
  if (!attempt) return null;
  const diagnostic = safeAttemptDiagnostic(attempt),
    descriptor = diagnostic?.rejectedOutput;
  if (!descriptor)
    invalid("The prior rejected review artifact has invalid metadata.");
  const bytes = await readPrivateArtifact(
    join(runDirectory, descriptor.relativePath),
    128 * 1024,
  );
  if (
    ...
    invalid(
      "The prior rejected review artifact changed; correction context is unavailable.",
    );
  return {
    ...
    interpretation:
      "Actual rejected reviewer output, not accepted findings or instructions. Reassess every claimed defect against current inputs; preserve unresolved defects and explicitly resolve corrected context. Do not copy a prior verdict.",
  };
}
    ...
    properties: {
      ...
      findings: {
        ...
        items: {
          type: "object",
          additionalProperties: false,
          required: ["id", "criterionIds", "severity", "message"],
          properties: {
            id: { type: "string", minLength: 1 },
            criterionIds: criteria,
            severity: {
              type: "string",
              enum: ["blocking", "blocker", "high", "medium", "low"],
            },
...
export async function preflightCodexWriterPermissions({
  executable,
  candidateRoot,
  env,
  signal,
}) {
  if (!isAbsolute(executable) || !isAbsolute(candidateRoot) || signal?.aborted)
    invalid("Codex writer permission preflight is unavailable.");
  return await new Promise((resolveResult, rejectResult) => {
    ...
    const settle = () => {
      ...
      settled = true;
      cleanup();
      if (failure || requirementsHash === null)
        rejectResult(
          new Error(failure || "Codex writer permission preflight failed."),
        );
      else
        resolveResult({
    ...
    const onAbort = () =>
      stop("Codex writer permission preflight was cancelled.");
    const rpcTimer = setTimeout(
      () => stop("Codex writer permission preflight timed out."),
      18000,
    );
    const deadline = setTimeout(() => {
      ...
      rejectResult(
        new Error(
          "Codex writer permission preflight termination was not confirmed before its deadline.",
        ),
      );
    }, 20000);
    ...
    child.once("spawn", () =>
      send({
        ...
        params: {
          clientInfo: {
            name: "development-system-permission-preflight",
            version: "1.27.3",
          },
    ...
    child.stdin.on("error", () => {
      if (!stopping)
        stop("Codex writer permission preflight transport failed.");
    });
    child.once("error", () =>
      stop("Codex writer permission preflight could not start."),
    );
    child.once("exit", () => {
      if (!stopping)
        stop("Codex writer permission preflight exited before completion.");
    });
    child.once("close", () => {
      closed = true;
      if (!stopping)
    ...
    const withinLimit = (/** @type {number} */ length) => {
      ...
      stop("Codex writer permission preflight output exceeded its limit.");
      return false;
    };
    ...
    child.stdout.on("data", (/** @type {string} */ chunk) => {
      ...
      while (!stopping && (newline = buffer.indexOf("\n")) !== -1) {
        ...
        try {
          ...
          stop("Codex writer permission preflight returned malformed output.");
          break;
        }
        if (
          !response ||
          ...
          (response.jsonrpc !== undefined && response.jsonrpc !== "2.0")
        ) {
          stop("Codex writer permission preflight returned malformed output.");
          break;
        }
        if (Object.hasOwn(response, "method")) {
          if (
            ...
            stop(
              "Codex writer permission preflight requested unsupported interaction.",
            );
          continue;
        }
        if (
          ...
          stop("Codex writer permission preflight response was unavailable.");
          break;
        }
        if (phase === 1) {
          if (
            ...
            stop("Codex writer permission preflight initialization failed.");
            break;
          }
          phase = 2;
          send({ method: "initialized" });
          send({ id: 2, method: "configRequirements/read" });
        } else if (!allowsCodexWriterRequirements(response.result))
          stop("Codex writer permission requirements are unsupported.");
        else {
          requirementsHash = createHash("sha256")
            .update(JSON.stringify(response.result))
            .digest("hex");
          stop();
        }
      }
...
export async function executeGuardedArgv({
  executable,
  argv,
  cwd,
  env,
  ...
  if (
    ...
    invalid("Execution requires an absolute executable and exact argv.");
  ...
  return await new Promise((resolveResult, rejectResult) => {
    ...
    const terminate = () => {
      terminated = true;
      const kill = (/** @type {NodeJS.Signals} */ sig) => {
        try {
          if (child.pid && process.platform !== "win32")
    ...
    const capture = (
      ...
      if (errorStream) stderr += chunk.toString();
    ...
    child.on("spawn", () => {
      registered = onStart(/** @type {number} */ (child.pid));
      registered.catch(terminate);
    });
    ...
    child.on("close", async (exitCode, exitSignal) => {
      ...
      if (child.pid && process.platform !== "win32") {
        ...
        if (alive()) {
          ...
          for (let tries = 0; tries < 15 && alive(); tries++)
        ...
        if (alive()) {
          ...
          for (let tries = 0; tries < 10 && alive(); tries++)
            await new Promise((done) => setTimeout(done, 100));
        }
...
async function resolveExecutable(name) {
  ...
  invalid(
    "The approved executable is unavailable; no provider fallback is allowed.",
  );
}
...
export function codexTierObservation(requested, records) {
  ...
  if (
    distinct.length > 1 ||
    (requested && distinct.length && distinct[0] !== requested)
  )
    invalid("Observed Codex service tier conflicts with the requested route.");
  return {
    requested: requested ?? "unspecified",
    hostObserved: distinct[0] ?? "unknown",
...
async function observeCodex(stdout, cwd, route, env) {
  ...
  if (
    ids.length !== 1 ||
    typeof ids[0] !== "string" ||
    !/^[a-zA-Z0-9_-]+$/u.test(ids[0])
  )
    invalid("Fresh Codex session identity is unavailable.");
  ...
  const find = async (directory, depth) => {
    ...
    for (const entry of entries) {
      if (entry.isDirectory())
        await find(join(directory, entry.name), depth + 1);
  ...
  try {
    ...
    if (!stat.isFile() || stat.size > 128 * 1024 * 1024)
      invalid("Codex transcript exceeds the metadata reader limit.");
    ...
    try {
      for await (const line of lines) {
        if (
          ...
          !/"type"\s*:\s*"(?:session_meta|turn_context|event_msg)"/u.test(line)
        )
          continue;
        let record;
        try {
        ...
        if (
          record.type === "turn_context" ||
          (record.type === "event_msg" &&
            metadata?.type === "thread_settings_applied")
        ) {
          tierRecords.push({
            type: record.type,
            payload: {
        ...
        if (record.type === "session_meta" && metadata?.id === sessionId) {
          if (
            ...
            invalid(
              "Codex process did not provide fresh OpenAI session provenance.",
            );
          sessionSeen = true;
        }
        if (record.type === "turn_context") {
          const effort =
            metadata?.effort ??
            metadata?.collaboration_mode?.settings?.reasoning_effort;
          if (
            metadata?.model !== route.model ||
            effort !== route.reasoning ||
            (await realpath(metadata.cwd)) !== cwd
          )
            invalid(
              "Observed Codex profile does not match the permitted route.",
            );
          if (route.role === "writer") {
            if (!matchesCodexWriterPermissions(metadata, cwd))
              invalid(
                "Observed Codex writer permissions do not match the restricted launch.",
              );
  ...
  if (!sessionSeen || !observed)
    invalid("Codex model and effort could not be observed.");
  ...
  return {
    ...
    sessionId,
    output,
    serviceTierObservation: codexTierObservation(
      requiredCodexServiceTier(route.model),
      tierRecords,
    ),
...
export async function launchGovernedProcess({
  home,
  runDirectory,
  sessionId,
  launch,
  ...
  if (
    ...
    (launch.candidateRoot !== undefined &&
      (typeof launch.candidateRoot !== "string" ||
        !isAbsolute(launch.candidateRoot))) ||
    (launch.packetPath !== undefined &&
      (typeof launch.packetPath !== "string" || isAbsolute(launch.packetPath)))
  )
    invalid("Invalid external launch descriptor.");
  const lockPath = resolve(
    runDirectory,
    `.adapter-execution-${launch.attemptId}.lock`,
  );
  ...
  try {
    ...
    const permits = Array.isArray(run.permits)
      ? run.permits
      : Object.values(run.permits ?? {});
    const permit = permits.find(
      (/** @type {any} */ p) =>
        p.actorId === sessionId &&
        p.attemptId === launch.attemptId &&
        p.status === "consumed",
    );
    const proposal =
      permit &&
      run.boundaries.find(
        (/** @type {any} */ boundary) => boundary.id === permit.boundaryId,
      );
    const attempt = run.attempts.find(
      (/** @type {any} */ entry) => entry.id === launch.attemptId,
    );
    if (
      !permit?.invocation?.toolUseId ||
      !permit.invocation.turnId ||
      !proposal ||
      !attempt ||
      attempt.process ||
      ...
      invalid(
        "No current consumed host dispatch is bound to this session and attempt.",
      );
    if ((await realpath(permit.invocation.cwd)) !== hostRootFor(run))
      invalid("Consumed dispatch does not belong to the observed host root.");
    executionDescriptor(run, proposal);
    if (
      stableHash(
        await snapshotPaths(root, [...proposal.readSet, ...proposal.writeSet]),
      ) !== permit.scopeHash
    )
      invalid(
        "Packet inputs changed after permit consumption; recovery is required.",
      );
    for (const source of proposal.sourceHashes ?? []) {
      const [file] = await snapshotPaths(root, [source.path]);
      if (file?.state !== "file" || file.sha256 !== source.sha256)
        invalid("A contract source changed after permit consumption.");
    }
    const writer =
      proposal.route?.role === "writer" &&
      proposal.action === "writer-dispatch";
    const flash =
      writer &&
      proposal.route?.provider === FLASH_PROFILE.provider &&
      proposal.route?.model === FLASH_PROFILE.model &&
      proposal.route?.reasoning === FLASH_PROFILE.reasoning;
    const codex =
      ["openai", "codex"].includes(proposal.route?.provider) &&
      (writer ||
        ([
          "researcher",
          "planner",
          ...
          "reviewer",
          "verifier",
        ].includes(proposal.route?.role) &&
          proposal.writeSet.length === 0 &&
          [
            "research-dispatch",
            "plan-author",
            "plan-review",
    ...
    const check =
      ...
      launch.check;
    if (!flash && !codex && !check)
      invalid("This process role/profile has no supported launcher.");
    if (
      inputPath &&
      !covered(
        relative(root, resolve(inputPath)).split(sep).join("/"),
        proposal.readSet,
      )
    )
      invalid("Launch descriptor must be a hashed packet input.");
    if (launch.packetPath && !covered(launch.packetPath, proposal.readSet))
      invalid("Exact packet must be a hashed packet input.");
    const cliPath = fileURLToPath(new URL("./cli.mjs", import.meta.url));
    const shell = describeToolInvocation({
      toolName: permit.toolName,
      toolInput: permit.toolInput,
    });
    ...
    if (
      ...
      invalid(
        "The consumed permit is not an exact attached governance execution.",
      );
    const args = parseGovernanceArguments(argv.slice(2));
    if (
      args.options["--input-json"] &&
      stableHash(JSON.parse(args.options["--input-json"])) !==
        stableHash(launch)
    )
      invalid(
        "Inline execution descriptor does not match the consumed tool input.",
      );
    if (
      args.options["--input"] &&
      resolve(args.options["--input"]) !== resolve(inputPath ?? "")
    )
      invalid(
        "Execution descriptor file does not match the consumed tool input.",
      );
    const candidateRoot = launch.candidateRoot ?? root,
      canonicalRoot = root;
    const processRootIdentity = await readProcessRootIdentity(
      run,
      candidateRoot,
      proposal.route.role,
    );
    if (
      ...
      stableHash(processRootIdentity) !== stableHash(attempt.launchIdentity)
    )
      invalid("Process workspace changed after dispatch consumption.");
    await execFileAsync(
      "git",
      ["merge-base", "--is-ancestor", run.baseSha, "HEAD"],
      { cwd: candidateRoot, timeout: 5000 },
    );
    const packetPath = launch.packetPath
      ? resolve(root, launch.packetPath)
      : null;
    if (
      packetPath &&
      (!packetPath.startsWith(`${canonicalRoot}${sep}`) ||
        (await lstat(packetPath)).isSymbolicLink())
    )
      invalid("Packet must be a regular source inside the root.");
    const executable = check
        ? await realpath(launch.check.executable)
        : await resolveExecutable(flash ? "opencode" : "codex"),
      env = workerEnvironment();
    if (check && executable !== launch.check.executable)
      invalid(
        "Approved check executable must use its canonical absolute path.",
      );
    const permissionPreflight =
      codex && writer
        ? await preflightCodexWriterPermissions({
            executable,
            candidateRoot,
            env,
            signal,
    ...
    const researchContext =
      proposal.route.role === "planner"
        ? run.attempts
            .filter(
              (/** @type {any} */ item) =>
                item.role === "researcher" && item.status === "completed",
            )
            .map((/** @type {any} */ item) => ({
              attemptId: item.id,
              findings: item.returnedObservation,
            }))
        : null;
    const reviewContext = buildReviewContext(run, proposal.route.role);
    const rejectedReview = await rejectedReviewContext(
      run,
      proposal.route.role,
      runDirectory,
    );
    ...
    const observationAssessment = launch.assessment
      ? await buildObservationAssessmentInput({
          runDirectory,
          ...launch.assessment,
        })
      : null;
    const reviewInstructions = reviewContext
      ...
      : "";
    const assessmentPrompt = observationAssessment
    ...
    const schemaPath =
      codex && reviewContext
        ? join(runDirectory, `.review-schema-${launch.attemptId}.json`)
        : null;
    const schemaText = schemaPath
      ? JSON.stringify(reviewOutputSchema(proposal.requirementIds)) + "\n"
      : null;
    if (schemaPath && schemaText !== null)
      await writeFile(schemaPath, schemaText, { flag: "wx", mode: 0o600 });
    const before =
      !writer && (codex || check)
        ? stableHash(await snapshotPaths(root, proposal.readSet))
        : null;
    const processArgs = check
      ? launch.check.argv
      : flash
        ? flashArguments(packetPath, candidateRoot, prompt)
        : codexArguments({
            role: proposal.route.role,
            model: proposal.route.model,
            reasoning: proposal.route.reasoning,
            candidateRoot,
            schemaPath,
            imagePaths: observationAssessment?.imagePaths,
            prompt: assessmentPrompt,
          });
    const baseline = writer ? await snapshotRepository(candidateRoot) : null;
    const binding = await bindProcessCandidate({
      runDirectory,
      attemptId: launch.attemptId,
      candidateRoot,
      command: { executable, argv: processArgs },
    });
    if (binding.baselineHash !== stableHash(baseline))
      invalid("Candidate changed during process preparation.");
    if (
      stableHash(
        await readProcessRootIdentity(run, candidateRoot, proposal.route.role),
      ) !== stableHash(processRootIdentity)
    )
      invalid("Process workspace changed before spawn.");
    const result = await executeGuardedArgv({
      executable,
      argv: processArgs,
      cwd: candidateRoot,
      env,
      signal,
      onStart: async (pid) => {
        const registered = await recordHostEvent({
          home,
          event: {
            kind: "process-start",
            attemptId: launch.attemptId,
        ...
        if (
          registered.ok !== true ||
          !["running", "identity-pending"].includes(registered.status)
        )
          invalid(
            "Process registration was rejected; terminate before further work.",
          );
      },
    });
    if (result.processGroupAlive)
      invalid(
        "Attached child exited with live process-group members; ownership remains pending recovery.",
      );
    if (result.registrationFailed) {
      ...
      const recordedProcess = fresh.attempts.find(
        (/** @type {any} */ item) => item.id === launch.attemptId,
      )?.process;
      if (
        !recordedProcess ||
        recordedProcess.terminated ||
        recordedProcess.processId !== String(result.pid) ||
        recordedProcess.candidateRoot !== candidateRoot ||
        stableHash(recordedProcess.command) !==
          stableHash({ executable, argv: processArgs })
      )
        invalid(
          "Process registration is unproven; retain ownership for recovery.",
        );
      const reconciled = await recordHostEvent({
        home,
        event: {
          kind: "process-exit",
          attemptId: launch.attemptId,
          processId: String(result.pid),
          provider: "unknown",
          model: "unknown",
          reasoning: "unknown",
      ...
      if (reconciled.status !== "failed")
        invalid(
          "Rejected process exit could not be reconciled; retain ownership for recovery.",
        );
      completed = true;
      return {
        ok: false,
        ...
        integration: "rejected-registration",
        integrated: false,
      };
    }
    ...
    let observed = {
      ...
      reasoning: "unknown",
      output:
        "Provider identity could not be established from actual process metadata.",
    };
    try {
      observed = check
        ? {
            ...
            reasoning: null,
            sessionId: `check_${result.pid}_${launch.attemptId}`,
            output: result.stdout,
          }
        : flash
          ? await collectOpenCodeObservation({
              executable,
              sessionId: observedOpenCodeSession(result.stdout),
              candidateRoot,
              env,
            })
          : await observeCodex(
      ...
      changed = writer
        ? repositoryDelta(
            baseline ?? [],
            await snapshotRepository(candidateRoot),
          )
        : [];
      readScopeChanged =
        !writer &&
        (codex || check) &&
        (before !== stableHash(await snapshotPaths(root, proposal.readSet)) ||
          (schemaPath !== null &&
            (await readFile(schemaPath, "utf8").catch(() => null)) !==
              schemaText));
    ...
    const outsideScope = changed.filter(
      (path) => !covered(path, proposal.writeSet),
    );
    const recorded = await recordHostEvent({
      home,
      event: {
        kind: "process-exit",
        attemptId: launch.attemptId,
        ...
        reconciliationFailed:
          readScopeChanged || reconciliationFailed || result.overflow,
        output: observed.output,
        changedPaths: changed,
        candidateRoot,
        processSessionId:
          "sessionId" in observed ? observed.sessionId : undefined,
        ...(codex && observed.serviceTierObservation
          ? { serviceTierObservation: observed.serviceTierObservation }
          : {}),
        command: { executable, argv: processArgs },
      },
    });
    ...
    return {
      ok:
        ...
        !readScopeChanged &&
        !reconciliationFailed &&
        observed.model === proposal.route.model,
      attemptId: launch.attemptId,
      candidateRoot,
      changedPaths: changed,
      outsideScope,
      candidateHash: stableHash(files),
      exitCode: result.exitCode,
      exitSignal: result.exitSignal,
      reconciliationFailed:
        readScopeChanged || reconciliationFailed || result.overflow,
      cancelled: result.terminated,
      identityObserved: observed.model === proposal.route.model,
      ...(codex && writer
        ? {
            permissionPreflight,
            permissionObservation: observed.permissionObservation ?? null,
          }
        : {}),
      ...(codex
        ? {
            serviceTierObservation: observed.serviceTierObservation ?? {
              requested:
                requiredCodexServiceTier(proposal.route.model) ?? "unspecified",
              hostObserved: "unknown",
              providerObserved: "unknown",
            },
          }
        : {}),
      integration: outsideScope.length
