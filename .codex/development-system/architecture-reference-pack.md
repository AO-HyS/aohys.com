...
An architecture-convergence Technical Grill is a repository-wide operating model, not a list of visible hotspots. Its frontmatter must declare:

```yaml
working_backwards_program: architecture-convergence
```

Before that Grill can be approved, it must contain a human-readable matrix with these exact dimension identifiers and four columns: `Dimension | Current evidence | Decision | Enforcement`.

| Dimension              | What the evidence and decision must settle                                                                                                                            |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| `migration-sequencing` | Dependency-aware slices that converge the whole agreed target while preserving behavior, data, compatibility, rollback, and explicit authorization gates.             |

Every row must state `keep`, `change`, `remove`, or `unproven` in ordinary language and name a concrete enforcement or evidence seam. `Unproven` is a research obligation, not permission to silently shrink scope. Product Contract, Technical Contract, and Implementation Map must carry every `change`, `remove`, and unresolved `unproven` row to an acceptance path, an explicit evidence-backed no-change conclusion, or a blocking gate. Do not leave a permanent catch-all “later” list.
